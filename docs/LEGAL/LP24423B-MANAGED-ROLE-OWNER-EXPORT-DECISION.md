...
## Raw-SQL diagnostic correction — September 9, 2026

The original diagnostic was **psql-only**: `\set`, `\gset`, `\gexec` and client variables generated SQL and preserved JSON across ROLLBACK in the client. Passing that file unchanged to a raw PostgreSQL connector cannot work. Its earlier psql fixture test established psql behavior only. The incompatible proposal was stopped before production access; no production diagnostic was executed.

`supabase/retention/diagnose-managed-role-raw.PROPOSAL.sql` is the separate connector proposal. Send its entire contents as **one raw query**, without splitting, wrapping, substitution or preprocessing. This correction authorizes no production execution. The legacy file remains clearly labelled psql-only. These instructions supersede the diagnostic execution guidance below and in the export-level record; export contracts and transition safeguards are unchanged.

The raw query uses only SET LOCAL, an anonymous invoker DO block, and one final SELECT. PostgreSQL generates `gridly_diag_` plus a UUID internally and creates that role with exactly NOLOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE NOINHERIT NOREPLICATION NOBYPASSRLS CONNECTION LIMIT 0. A collision aborts; no existing role is reused. All observation reads are from system catalogs. The real monitor, reset authorization, project identity and application tables are absent from the SQL.

**Observation and cleanup proof:** the role is created inside a PL/pgSQL exception subtransaction. After observation, a private sentinel exception rolls back that subtransaction, including the role and automatic membership. PL/pgSQL variables retain their values. The code then independently queries the exact generated name and refuses an incomplete observation or surviving role. Complete JSON is encoded as one quoted identifier in the existing `search_path` setting using `set_config(..., true)`, solely as transaction-local transport. The final fully `pg_catalog`-qualified SELECT decodes it with `parse_ident` and returns one JSON column/row. It never resolves that identifier as a schema. No helper function, table, schema or custom setting is created. The single-query implicit transaction restores search_path and both timeouts on completion or failure. Do not split statements into separate requests or keep an outer transaction open.

The result includes role identity; all seven privilege/login/inheritance flags, connection limit, role and per-database configuration presence without values; every inbound/outbound membership with direction, role/member, ADMIN/INHERIT/SET, grantor and grantor-superuser flag; effective CONNECT/CREATE/TEMP for every database and USAGE/CREATE for every schema in the connected database; explicit schema/relation/column/function/database grant counts plus all ACL dependency counts; total and per-catalog owned-object counts; authenticator presence and effective SET capability; predicate booleans, status, cleanup success and exact-name absence. Password columns, configuration values and application contents are never returned. Configuration absence, zero grants and zero ownership are checked at the pre-grant stage; the diagnostic grants no health-view access.

`memberships_match` retains the repaired predicate's literal `postgres` member identity and superuser grantor requirement. `creator_membership_shape_match` separately checks the actual executing creator with the same direction/options/grantor constraints. `matches_repaired_production_predicate` combines the role attribute, exact production membership and pre-grant privilege checks. Completeness additionally requires exactly one automatic edge, present authenticator, nonempty database/schema observations and no null observed catalog fields. A separate conservative effective-privilege gate rejects database/schema CREATE and nonstandard schema USAGE; PUBLIC CONNECT/TEMP and standard catalog/information_schema/public USAGE are observed without granting anything. Unexpected PUBLIC privileges require review, even if the narrower repaired predicate matches. No PUBLIC allowance is silently added for a managed platform.

An unexpected attribute, membership shape, grant, ownership, configuration, effective privilege, missing observation or cleanup failure raises an error and yields no success row. Errors after observation include only the safe catalog JSON in DETAIL; they do not authorize retry. A complete compatible observation made by a synthetic creator returns `status=NO-GO` and `matches_repaired_production_predicate=false`, because that creator is not the production `postgres` owner. Only a complete, untruncated `PASS` with cleanup/absence true can support a separate owner review; an error, NO-GO, missing field or truncation stops the attempt.

**Local verification:** `node --test tests/lp24423b-raw-diagnostic.test.cjs` uses the existing PostgreSQL **17.10** listener at **127.0.0.1:55441**, a disposable database and a synthetic non-superuser CREATEROLE login. Login is needed to send the file unchanged without prepending SET ROLE. The exact file is transmitted in one PostgreSQL wire-protocol Q message, with no psql parser. Returned DataRows are collected through idle ReadyForQuery, proving query completion. Tests parse one complete JSON row larger than an identifier, preserve quoted metadata, verify automatic role → creator ADMIN=true/INHERIT=false/SET=false with grantor `postgres` (the local bootstrap superuser), repeat exact execution with distinct UUIDs, and query absence independently afterward. A subsequent query on the **same connection** verifies original search_path/statement_timeout/lock_timeout values are restored. The fixture deliberately reports NO-GO for production identity; this is not evidence about production and does not exercise production's PASS identity.

Negative tests cover login, CREATEDB, CREATEROLE, inheritance, connection limit, wrong membership direction/options/API edge, table/column/schema grants, ownership, role/per-database settings without value disclosure, missing observation, role-name collision, unexpected PUBLIC schema USAGE, errors immediately after creation and after observation, corrupted result transport, and deliberately bypassed inner rollback. Each failing query yields zero rows and independent catalog checks show zero diagnostic roles. Schema identity/owner/ACL inventory and fixture data remain unchanged. There are no persistent helper functions or tables. A fresh-cluster initdb attempt was blocked by Windows restricted-token handling; testing then used the existing local fixture, without escalation or production access.

Final focused result: **17 tests passed, 0 failed, 0 skipped**. Raw diagnostic canonical-LF SHA-256: `e450daa5adfc20e167513afa585951eb8e1957b4bf0eadfe7662b96130b58106`. Independent final checks found zero diagnostic roles, transport tables or helper functions; schema/ACL inventory and synthetic data were unchanged. Ready for a new owner-reviewed local commit, with the production-identity and live-connector limitations above. This is a local implementation correction only; no staging, commit, Supabase access, approval request or authorization change is performed.
