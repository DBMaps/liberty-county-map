...
### Home versus Current View ownership evidence

Ownership separation is a product contract, not a value-inequality test. `homeAndCurrentViewDistinct` means the Home and Current View ownership contracts are distinct and safely preserved; it does not mean the current values must be different. `currentViewMatchesHome` is the descriptive value-comparison field: it is `true` when saved Home Dayton and Current View Dayton match, `false` when saved Home Dayton is viewed through county-wide Liberty County, and `null` when runtime evidence is insufficient. Current View may legitimately equal Home, may be another community, or may be county-wide. The decisive evidence is canonical Home preservation in `gridlyHomePersonalizationV1`, reported through `canonicalHomePreservedDuringCurrentView` and detailed in `homeCurrentViewFindings`.

`homeCurrentViewEvidenceStatus: "pass"` requires a valid canonical Home and observed separate ownership behavior, including either Home-equals-Current-View preservation or county-wide browsing that does not rewrite the saved Home. `"fail"` is reserved for proven overwrite or shared-owner evidence, such as county-wide browsing replacing saved Dayton / `77535` or a Current View change invalidating canonical Home. `"unobserved"` means ownership proof is insufficient. The launch gate adds `home_current_view_ownership_failed` only for `"fail"`, adds `home_current_view_ownership_unobserved` only for `"unobserved"`, and adds no ownership blocker or pending item for `"pass"`.
