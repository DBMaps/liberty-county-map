...
## Fallback rejection evidence propagation repair

### Production trace correction (LP102.2)

The post-PR #2712 production observation was `result.cases[0].resolutionEvents === undefined`. A complete property trace found the concrete in-browser loss point: `fetchGridlyNominatimSearch` placed the Edge rejection array on its event, but `recordGridlyDestinationProviderEvent` rebuilt that event from an allow-list which omitted `fallbackCandidateDiagnostics`. Consequently `gridlyDestinationProviderState.lastDiagnostics.variants[]` no longer contained the array, and the investigation case could not project it into `resolutionEvents`. The candidate gate, canonical result, and cache were not the loss point.

The authoritative contract is now: Edge HTTP and cached payload property `diagnostics`; browser-client internal property `diagnostics`; app session property `providerDiagnostics.variants[]`; investigation property `cases[].resolutionEvents[]`; and certification lookup `cases[].resolutionEvents[].fallbackCandidateDiagnostics[]`. The Edge field is named `primaryProviderOutcome` at creation rather than being renamed later. The browser client's `diagnostics` property is non-enumerable, is copied separately from result candidates, and defensively omits rejection records for ordinary `explicit_search` calls. No rejection record enters any consumer candidate array or UI.

Diagnostic cache keys already differed by `requestMode`. This repair additionally adds the narrow `lp102-rejection-v2` diagnostic-contract dimension, so a pre-repair LP102 entry cannot mask new evidence. Cache writes store the whole canonical payload and cache reads spread the stored response without schema projection, preserving `diagnostics.fallbackCandidateDiagnostics`; the consumer cache namespace and caching behavior are unchanged.

The passive `window.gridlyLp102DiagnosticPathTrace()` helper reports the last already-observed path and never makes a request. Deployment requires an Edge Function redeploy, deployment of both changed browser scripts, and a hard refresh. It does not require a global cache namespace change. Production certification remains outstanding; therefore this repository evidence alone is not a merge recommendation. After deployment, require the focused investigation to show the 274-versus-698 rejection record, then require certification to return `failedChecks: []` and `safeToMerge: true` before recommending merge.

Production already confirmed the safe consumer outcome for `274 County Road 677, Dayton, TX 77535`: the primary and fallback outcomes were confirmed no-result, every consumer candidate/render collection was empty, Route Preview was unavailable, and `698 CO RD 677` was not shown. The remaining defect was evidentiary. The fallback acceptance gate discarded its evaluated candidate diagnostics from the response path used by the browser, so `resolutionEvents[].fallbackCandidateDiagnostics` was empty and certification could not prove why the safe no-result occurred.

LP102 investigation and certification calls now use the explicit, non-privileged `lp102_certification` request mode. This marker grants no provider or acceptance bypass: it has the same fallback eligibility and candidate gate as `explicit_search`; it only asks the canonical response to retain a minimal rejected-candidate comparison. Ordinary consumer searches remain `explicit_search` and omit `fallbackCandidateDiagnostics`. The browser copies the records only into resolution/rejection evidence and never into `candidates`, `providerCandidates`, `candidatePipeline`, relevance, rendering, or Route Preview.

The diagnostic allow-list consists only of disposition/rule/stage/phase, blocking conflicts, requested and returned normalized house/road values and agreements, requested and returned state/ZIP/county values and agreements, result type, precision, and a false Route Preview eligibility flag. It excludes coordinates, full matched addresses, provider identity/TIGER metadata, raw payloads, URLs, request headers, credentials, cache identifiers, and stack traces.

Diagnostic mode participates in the existing normalized cache key. It therefore has a separate short-lived no-result cache entry from ordinary consumer mode: a stale consumer no-result cannot erase certification evidence, repeated certification variants retain normal cache/in-flight coalescing, and global caching is not disabled. No cache namespace change is required.

Certification treats the rejected mismatch as valid safety evidence only when 274 and 698 are both explicit and unequal, house agreement is false, the rule/disposition/stage/phase are the governed mismatch values, Route Preview eligibility is false, and the target remains absent from canonical results, provider candidates, relevance output, final render input, visible DOM, and Route Preview. `ruralPrecisionTruthful` now accepts either a truthfully labeled retained rural result or this proven unsafe-interpolation rejection followed by a truthful no-result. Missing or contradictory evidence continues to fail closed and prevents `safeToMerge`.

The Edge Function and cache-busted browser assets must be deployed before production validation. After a hard refresh, run the focused investigation and full certification commands below and require a non-empty 274-versus-698 diagnostic, no consumer candidate, unavailable Route Preview, `failedChecks: []`, and `safeToMerge: true`. Until that production browser evidence exists, automated contracts alone do **not** support a merge recommendation.
...
That earlier scope statement is superseded by the fallback rejection evidence propagation repair above. Consumer destination behavior, candidate acceptance, provider selection, ranking, rendering, Route Preview, and business/governed precedence remain unchanged, but the canonical diagnostic response and browser evidence plumbing have changed. The Edge Function must therefore be redeployed, and the updated browser JavaScript must be deployed and cache-busted (or loaded with a hard refresh) before repeating:
