...
The onboarding phase activates the installed app without reset, captures each semantically identified visible page, presses Next automatically, and stops after capturing page 7, **Set your awareness area**. The seven source titles are Welcome to Gridly; Know Before You Go; See what's happening nearby; Stay informed with important updates; Your report helps everyone nearby; Make Gridly yours; Set your awareness area. It neither presses Use My Location nor enters a Home choice. It records a partial run if onboarding already starts later, and never resets/replays to fabricate a fresh run.
...
`console` explicitly relaunches Gridly and attaches console collection for up to 30 seconds. Its timeout is a collection limit, not a passing runtime test. No existing device log is cleared. Screenshot destination flags are selected only when documented in installed command help; an unknown syntax stops with help preserved. CoreDevice JSON paths are explicit adapters covered by fixtures, **not yet confirmed against the installed Mac's JSON**. Unknown schema/readiness stops input automation and requires adapting the harness to the saved actual output; never relax an unknown field into PASS.
