...
function collectReportVisibilityAudit() {
  const selectors = ["#gridlyHazardPanel", "#reportSection", "#mobileDailyPanel", "#mobileNativeSurfaceLayer"];
  const read = (selector) => {
    const el = document.querySelector(selector);
    const computed = el ? window.getComputedStyle(el) : null;
    const rect = el?.getBoundingClientRect?.() || null;
    ...
    while (current && current !== document.documentElement) {
      ...
      if (hidden) hiddenAncestors.push(current.id ? `#${current.id}` : current.className || current.tagName.toLowerCase());
      current = current.parentElement;
    }
    return {
      selector,
      exists: Boolean(el),
      hiddenAttr: el ? Boolean(el.hidden) : null,
      ...
      translateY: computed?.translate || null,
      height: computed?.height || null,
      maxHeight: computed?.maxHeight || null,
      overflow: computed?.overflow || null,
      overflowY: computed?.overflowY || null,
      clipPath: computed?.clipPath || null,
      zIndex: computed?.zIndex || null,
      rect: rect ? { top: rect.top, left: rect.left, width: rect.width, height: rect.height, bottom: rect.bottom } : null,
      hiddenAncestors
    };
  };
  return { viewport: { width: window.innerWidth, height: window.innerHeight }, surfaces: selectors.map(read) };
}
...
window.gridlyReportActionTraceDebug = function gridlyReportActionTraceDebug() {
  return {
    ...
    visibilityAudit: collectReportVisibilityAudit(),
...
function handleReportNearMe(entryPoint = "report_near_me") {
  pushReportActionTrace("handleReportNearMe starts", {
    ...
    visibilityAuditBefore: collectReportVisibilityAudit()
  });
  const mobileSurfaceLayer = document.getElementById("mobileNativeSurfaceLayer");
  if (mobileSurfaceLayer && !mobileSurfaceLayer.hidden) {
    mobileSurfaceLayer.hidden = true;
    mobileSurfaceLayer.setAttribute("aria-hidden", "true");
    pushReportActionTrace("mobileNativeSurfaceLayer hidden for report visibility", {
      reason: "surface_layer_was_visible",
      mobileSurfaceLayer: collectReportActionTraceElementState("#mobileNativeSurfaceLayer")
    });
  }
  ...
  pushReportActionTrace("final visible/open state after attempted open", {
    ...
    visibilityAuditFinal: collectReportVisibilityAudit()
  });
}
