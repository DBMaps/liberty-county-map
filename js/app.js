...
(() => {
  ...
  function getRouteSurfaceSnapshot() {
    const places = (typeof normalizeSavedPlaces === "function" ? normalizeSavedPlaces() : []) || [];
    const startId = els?.routeWatchStartSelect?.value || "";
    const destinationId = els?.routeWatchDestinationSelect?.value || "";
    const start = places.find((place) => place.id === startId) || null;
    const destination = places.find((place) => place.id === destinationId) || null;
    ...
    const readinessState = start && destination ? "ready" : "needs_setup";
    const readinessMessage = readinessState === "ready" ? "Route Watch ready" : "Select Home and Destination";
  ...
  function triggerV2DockAdapter(action, payload = {}) {
    const bridges = {
      ...
      "route-watch-open": () => triggerV2DockAdapter("route-open"),
      "route-preview-open": () => triggerV2DockAdapter("route-open"),
      "route-manage-places-open": () => triggerV2DockAdapter("route-open"),
