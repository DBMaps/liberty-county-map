...
(() => {
  ...
  const sheetTemplates = {
    ...
    route: { title: "Route Setup", html: `<div class="gridly-v2-list"><p class="gridly-v2-sheet-copy">Route Watch adapters now open canonical route flows.</p><button class="primary-btn" data-v2-action="route-open" type="button">Open Route Watch</button><button class="secondary-btn" data-v2-action="route-open" type="button">View Route</button><button class="secondary-btn" data-v2-action="route-open" type="button">Manage Places</button></div>` },
    alerts: { title: "Alerts", html: `<div class="gridly-v2-list"><p class="gridly-v2-sheet-copy">Alerts adapters now use canonical intelligence rendering.</p><button class="primary-btn" data-v2-action="alerts-open" type="button">Open Alerts Center</button><button class="secondary-btn" data-v2-action="settings-open" type="button">Manage Alerts</button></div>` },
    settings: { title: "Settings", html: `<div class="gridly-v2-list"><button class="gridly-v2-tile" data-v2-action="settings-open" type="button">Set Home Town</button><button class="gridly-v2-tile" data-v2-action="settings-open" type="button">Home / Work / Saved Places</button><button class="gridly-v2-tile" data-v2-action="settings-open" type="button">Alert Preferences</button><button class="gridly-v2-tile" data-v2-action="settings-open" type="button">Route Preferences</button><button class="gridly-v2-tile" data-v2-action="settings-open" type="button">App Preferences</button></div>` }
  };
  const v2DockAdapterState = { lastDockActionTriggered: "", lastDockAdapterTarget: "", adapterBridgeFailures: [] };
  let activeSheet = "";
  ...
  function openPortraitV2Sheet(type) {
    ...
    activeSheet = type; title.textContent = template.title; body.innerHTML = template.html; bindV2SheetActions(); sheet.hidden = false; backdrop.hidden = false;
  }
  ...
  function triggerV2DockAdapter(action, payload = {}) {
    const bridges = {
      ...
      "route-open": () => document.getElementById("mobileDockRouteBtn")?.click(),
    ...
    v2DockAdapterState.lastDockAdapterTarget = action ? `canonical:${action}` : "";
    if (typeof bridge !== "function") {
      v2DockAdapterState.adapterBridgeFailures.push({ action, reason: "missing_bridge", at: Date.now() });
      return;
    }
    try { bridge(); closePortraitV2Sheet(); applyPortraitV2SurfaceContainment(); } catch (error) {
  ...
  function bindV2SheetActions() {
    ...
    body.querySelectorAll("[data-v2-action]").forEach((button) => {
      button.addEventListener("click", () => triggerV2DockAdapter(button.dataset.v2Action || "", { hazardType: button.dataset.hazardType || "" }));
    });
