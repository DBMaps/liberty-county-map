(function gridlyGovernedAwarenessModule(root, factory) {
  ...
  const VERSION = "LP219-governed-awareness-v1";
  ...
  function buildSnapshot(input = {}) {
    ...
    const actualSets = Object.fromEntries(SURFACES.map((surface) => [surface, new Set((actual[surface] || []).map(text))]));
    const seen = new Set();
    const duplicates = [];
    const evidence = [];
    for (const record of Array.isArray(input.records) ? input.records : []) {
      ...
      const eligible = Object.fromEntries(SURFACES.map((surface) => [surface, current && geographicEligible && policy[surface] === true]));
      ...
      evidence.push(Object.freeze({
        ...
        freshness: current ? "current" : "stale", current, geographicEligible,
    ...
    return Object.freeze({
      ...
      omissions: Object.freeze(evidence.flatMap((row) => SURFACES.filter((surface) => !row.published[surface]).map((surface) => Object.freeze({ evidenceId: row.evidenceId, surface, reason: row.omissionReasons[surface] })))),
