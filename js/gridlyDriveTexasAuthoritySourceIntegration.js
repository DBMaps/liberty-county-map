(function initGridlyDriveTexasAuthoritySourceIntegration(globalScope) {
  ...
  function selectedAnchor(area) {
    // Canonical PLACE identity resolves through the existing LP201 presentation
    // authority. County membership is governance context, never an anchor.
    const canonicalKey = text(area?.canonicalKey || area?.key);
    const explicitPlaceGeoid = text(area?.placeGeoid || area?.communityId);
    const isCanonicalPlace = /^place-48\d{5}$/.test(canonicalKey) || /^48\d{5}$/.test(explicitPlaceGeoid);
    if (isCanonicalPlace) {
      const focus = typeof globalScope.resolveGridlyCanonicalPlacePresentationFocus === "function"
        ? globalScope.resolveGridlyCanonicalPlacePresentationFocus(area)
        : null;
      const lat = num(focus?.lat), lng = num(focus?.lng);
      const configuredRadius = num(area?.radiusMiles ?? area?.awarenessRadiusMiles ?? area?.configuredAwarenessRadiusMiles ?? area?.radius);
      const radius = Number.isFinite(configuredRadius) ? configuredRadius : num(focus?.radiusMiles);
      return { lat, lng, radiusMiles: Number.isFinite(radius) ? radius : 7, valid: inTexas(lat, lng), authority: focus?.authority || null, canonicalKey: focus?.canonicalKey || canonicalKey || null };
    }
    ...
    return { lat, lng, radiusMiles: Number.isFinite(radius) ? radius : 7, valid: inTexas(lat, lng), authority: inTexas(lat, lng) ? "SELECTED_AWARENESS_AREA_COORDINATE" : null, canonicalKey: null };
  }
  ...
  function select(input = {}) {
    ...
    const resolvedAwarenessAnchor = selectedAnchor(activeAwarenessArea);
  ...
  function gridlySelectConsumerVisibleDriveTexasSituations(input = {}) {
    // A caller that is building a source-status envelope must be able to
    // project the exact authority evaluation it just inspected.  Re-running
    // the selector here used to permit the selected area/source cache to move
    // between LP039.2 and LP039.3 (the live Houston 6 -> 0 failure).
    const suppliedSnapshot = input && input.authoritySnapshot;
    const snap = suppliedSnapshot && typeof suppliedSnapshot === "object"
      ? suppliedSnapshot
      : (typeof globalScope.gridlyGetDriveTexasAuthoritySnapshot === "function" ? globalScope.gridlyGetDriveTexasAuthoritySnapshot(input) : null);
    ...
    return freeze({
      ...
      evaluationRevision: snap?.evaluationRevision || null,
