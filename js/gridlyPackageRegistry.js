(function initGridlyPackageRegistry(globalScope) {
  ...
  function validateIntelligenceFoundation(registryRef) {
    ...
    const runtimeOwnershipMigrated = intelligencePackages.some((pkg) => pkg.id !== "intelligence.community-reports" && pkg.intelligence?.runtimeOwnershipActive === true);
    const providerMigrationComplete = intelligencePackages.some((pkg) => pkg.id !== "intelligence.community-reports" && pkg.intelligence?.providerMigrationComplete === true);
    ...
    const invalidPackages = intelligencePackages.filter((pkg) => (pkg.id === "intelligence.community-reports" ? pkg.status !== "controlled-migrated" : pkg.status !== "planned-foundation") || pkg.validationState !== "valid" || pkg.intelligence?.validationState !== "valid");
    return Object.freeze({
      valid: intelligencePackages.length === 5 && communityReportsControlledMigrated === true && runtimeOwnershipMigrated === false && providerMigrationComplete === false && trustModelActivated === false && freshnessModelActivated === false && confidenceModelActivated === false && invalidPackages.length === 0,
      operationalRegion: SOUTHEAST_TEXAS_OPERATIONAL_REGION,
