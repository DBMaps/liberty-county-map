(function (root) {
  ...
  function renderSurface() { if (!gateEnabled() || !root.document || root.document.getElementById("gridlyPoiNonProductionSurface")) return; const host = root.document.querySelector(".gridly-search-card"); if (!host) return; const section = root.document.createElement("section"); section.id = "gridlyPoiNonProductionSurface"; section.className = "gridly-poi-nonproduction"; section.setAttribute("aria-label", "Non-production nearby places"); section.innerHTML = `<div class="gridly-poi-heading"><strong>Nearby places preview</strong><span>Non-production</span></div><div class="gridly-poi-controls"><label>Location<select id="gridlyPoiCohort">${Object.keys(COHORTS).map(name => `<option>${name}</option>`).join("")}</select></label><label>Radius<select id="gridlyPoiRadius"><option>5</option><option>10</option><option>25</option></select></label><label>Category<input id="gridlyPoiCategory" placeholder="All categories"></label><button id="gridlyPoiSearch" type="button">Find places</button></div><div id="gridlyPoiNonProductionResults" class="gridly-poi-results" aria-live="polite"></div><a class="gridly-poi-attribution" href="#dataSourcesAndLicenses">POI data sources and licenses</a>`; host.append(section); section.querySelector("#gridlyPoiSearch").addEventListener("click", async () => { const button = section.querySelector("#gridlyPoiSearch"); button.disabled = true; try { await search(requestForCohort(section.querySelector("#gridlyPoiCohort").value, section.querySelector("#gridlyPoiRadius").value, section.querySelector("#gridlyPoiCategory").value.trim().toUpperCase())); } catch (error) { state.providerFailure = `${error.stage || "PROVIDER"}: ${error.message}`; section.querySelector("#gridlyPoiNonProductionResults").textContent = "Nearby places could not be loaded. No alternate source was used."; } finally { button.disabled = false; } }); }
  const api = Object.freeze({ initialize, search, rollback, requestForCohort, audit, EXPECTED, _test: Object.freeze({ validateManifest, validateRecord, validateRequest, candidateShardIds, distanceMiles }) });
