(function (root) {
  ...
  const gateEnabled = () => root.__GRIDLY_POI_NON_PRODUCTION__ === true;
  ...
  async function initialize() {
    if (!gateEnabled()) { rollback(); fail("NON_PRODUCTION_GATE_OFF", "provider is disabled"); }
  ...
  function requestForCurrentContext(radiusMiles, category) {
    const context = root.gridlyGetCurrentGovernedLocationContext?.();
  ...
  function renderSurface() { if (!gateEnabled() || !root.document || root.document.getElementById("gridlyPoiNonProductionSurface")) return; const host = root.document.querySelector(".gridly-search-card"); if (!host) return; const section = root.document.createElement("section"); section.id = "gridlyPoiNonProductionSurface"; section.className = "gridly-poi-nonproduction"; section.setAttribute("aria-label", "Non-production nearby places"); section.innerHTML = `<div class="gridly-poi-heading"><strong>Nearby places</strong><span>Non-production</span></div><p id="gridlyPoiContextLabel" class="gridly-poi-context"></p><div class="gridly-poi-controls"><label>Radius<select id="gridlyPoiRadius"><option>5</option><option>10</option><option>25</option></select></label><label>Category<input id="gridlyPoiCategory" placeholder="All categories"></label><button id="gridlyPoiSearch" type="button">Find places</button></div><div id="gridlyPoiNonProductionResults" class="gridly-poi-results" aria-live="polite"></div><a class="gridly-poi-attribution" href="#dataSourcesAndLicenses">POI data sources and licenses</a>`; host.append(section); refreshSurfaceContext(section); root.document.addEventListener("click", event => { if (!section.contains(event.target)) refreshSurfaceContext(section); }); section.addEventListener("focusin", () => refreshSurfaceContext(section)); section.querySelector("#gridlyPoiSearch").addEventListener("click", async () => { const button = section.querySelector("#gridlyPoiSearch"); const request = requestForCurrentContext(section.querySelector("#gridlyPoiRadius").value, section.querySelector("#gridlyPoiCategory").value.trim().toUpperCase()); if (!request) { refreshSurfaceContext(section); section.querySelector("#gridlyPoiNonProductionResults").textContent = ""; return; } section.querySelector("#gridlyPoiContextLabel").textContent = `Nearby places around ${request.name}`; button.disabled = true; try { await search(request); } catch (error) { state.providerFailure = `${error.stage || "PROVIDER"}: ${error.message}`; section.querySelector("#gridlyPoiNonProductionResults").textContent = "Nearby places could not be loaded. No alternate source was used."; } finally { button.disabled = false; } }); }
  const api = Object.freeze({ initialize, search, rollback, requestForCohort, requestForCurrentContext, audit, EXPECTED, _test: Object.freeze({ validateManifest, validateRecord, validateRequest, candidateShardIds, distanceMiles }) });
  root.GridlyPoiBrowserProvider = api; root.gridlyPoiBrowserRehearsalAudit = audit;
  if (root.document) { if (root.document.readyState === "loading") root.document.addEventListener("DOMContentLoaded", () => { if (gateEnabled()) initialize().catch(() => renderSurface()); }, { once: true }); else if (gateEnabled()) initialize().catch(() => renderSurface()); }
