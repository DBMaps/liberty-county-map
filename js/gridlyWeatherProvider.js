(function initGridlyWeatherProvider(globalScope) {
  ...
  function normalizeRecord(record, index) {
    ...
    return freeze({
      ...
      event: toSafeString(record.event) || category,
      headline: headline || category,
      status: toSafeString(record.status) || null,
      messageType: toSafeString(record.messageType) || null,
      certainty: toSafeString(record.certainty) || null,
      urgency: toSafeString(record.urgency) || null,
      senderName: toSafeString(record.senderName) || null,
      response: toSafeString(record.response) || null,
      areaDesc: toSafeString(record.areaDesc) || null,
      affectedZones: freeze((Array.isArray(record.affectedZones) ? record.affectedZones : []).filter((zone) => typeof zone === "string" && zone.trim())),
      instruction: toSafeString(record.instruction) || null,
