...
assert.match(app, /"montgomery-tx": Object\.freeze\(\{[\s\S]*?operational: false,[\s\S]*?productionEnabled: false,[\s\S]*?selectable: false,[\s\S]*?runtimeGateEnabled: GRIDLY_MONTGOMERY_RUNTIME_GATE/, 'Montgomery registry status must remain disabled, production-disabled, non-selectable, and gate-bound');
...
assert.equal(stagedRuntimeValidation.finalDetermination, 'STAGED RUNTIME VALIDATION COMPLETE WITH OBSERVATIONS');
assert.equal(evidence.protectedSystemValidation.supabaseChanged, false);
assert.equal(readiness.readinessForFutureObservationPeriod, true);
