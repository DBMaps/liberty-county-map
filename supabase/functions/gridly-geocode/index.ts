...
async function execute(body: any, key: string, requestId: string, origin: string, db: any): Promise<Response> {
  ...
  const runtimeAddressDiagnostics = (responseSource: string, fallbackExecuted = false) => {
    ...
    return { version: "LP105.5", completedStages: completedStages.slice(0, 16),
      ...
      sidecarUsed: provider.sidecarUsed === true, sidecarObjectPath: provider.sidecarObjectPath || null,
      sidecarCertificateObjectPath: provider.sidecarCertificateObjectPath || null,
      sidecarCertificateValidated: provider.sidecarCertificateValidated === true,
      sidecarSourcePackageSha256Validated: provider.sidecarSourcePackageSha256Validated === true,
      sidecarDownloadStarted: provider.sidecarDownloadStarted === true, sidecarDownloadCompleted: provider.sidecarDownloadCompleted === true,
      sidecarBytesRead: Number(provider.sidecarBytesRead)||0, sidecarExpectedBytes: Number(provider.sidecarExpectedBytes)||0,
      sidecarByteSizeValidated: provider.sidecarByteSizeValidated === true, sidecarCalculatedSha256: provider.sidecarCalculatedSha256 || null,
      sidecarSha256Validated: provider.sidecarSha256Validated === true, sidecarOpened: provider.sidecarOpened === true,
      sidecarRecordsScanned: Number(provider.sidecarRecordsScanned)||0, sidecarExactLookupExecuted: provider.sidecarExactLookupExecuted === true,
      sidecarExactMatchFound: provider.sidecarExactMatchFound === true, sidecarFailureStage: provider.sidecarFailureStage || null,
      sidecarElapsedMilliseconds: Number(provider.sidecarElapsedMilliseconds)||0,
      sidecarManifestUsed: provider.sidecarManifestUsed === true, sidecarManifestValidated: provider.sidecarManifestValidated === true,
      sidecarSourcePackageIdentityValidated: provider.sidecarSourcePackageIdentityValidated === true,
      selectedBucketId: provider.selectedBucketId || null, selectedBucketObjectPath: provider.selectedBucketObjectPath || null,
      selectedBucketExpectedBytes: Number(provider.selectedBucketExpectedBytes)||0, selectedBucketBytesRead: Number(provider.selectedBucketBytesRead)||0,
      selectedBucketByteSizeValidated: provider.selectedBucketByteSizeValidated === true, selectedBucketCalculatedSha256: provider.selectedBucketCalculatedSha256 || null,
      selectedBucketSha256Validated: provider.selectedBucketSha256Validated === true, selectedBucketRecordCount: Number(provider.selectedBucketRecordCount)||0,
      selectedBucketRecordsScanned: Number(provider.selectedBucketRecordsScanned)||0, selectedBucketOpened: provider.selectedBucketOpened === true,
      selectedBucketExactMatchFound: provider.selectedBucketExactMatchFound === true, selectedBucketElapsedMilliseconds: Number(provider.selectedBucketElapsedMilliseconds)||0,
