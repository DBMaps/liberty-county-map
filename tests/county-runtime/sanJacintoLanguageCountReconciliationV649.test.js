...
['topAwarenessText', 'alertPanelText', 'locationCardText', 'visibleMarkerCount', 'visibleAlertCount', 'visibleLocationCardActiveCount', 'visualSurfaceReconciliationPass'].forEach((field) => {
  ...
  assert.match(
    source,
    new RegExp(`function gridlySanJacintoReportSubmissionAudit\\(\\) \{[\\s\\S]*${field}: domVisibleSurfaces\\.${field}`),
    `${field} is exposed directly from gridlySanJacintoReportSubmissionAudit output`
  );
});
