...
const saveFunction = extractFunction('saveGridlyHomeTownPreference');
const applyFunction = extractFunction('applyGridlyHomeTownAwarenessContext');
...
assert(!selectFunction.includes('gridlySetActiveCountyContext'), 'settings selection does not duplicate canonical county ownership');
assert(!selectFunction.includes('gridlyDispatchSemanticCamera'), 'settings selection does not duplicate canonical camera ownership');
assert(saveFunction.includes('gridlySetActiveCountyContext(resolvedCountyId'), 'canonical save updates active county context');
assert(saveFunction.includes('applyGridlyHomeTownAwarenessContext({ source: syncSource, fitMap: options.preserveSemanticCamera !== true })'), 'canonical save applies the selected awareness context');
assert(applyFunction.includes('gridlyDispatchSemanticCamera(homeTownAnchor, homeTownAnchor.countyId, { source })'), 'canonical awareness application dispatches the semantic camera');
...
assert(
  selectFunction.indexOf('saveGridlyHomeTownPreference(saveValue, { source })') < selectFunction.indexOf('gridlyRefreshSettingsAwarenessDisplayOnly(saved);'),
  'Dayton -> Liberty regression: canonical transition completes before the settings summary refreshes'
);
