...
assert.match(source, /awareness_area_display_name/, 'governed awareness-area label is the fallback before selected area');
...
assert.match(source, /Right now falls within a commonly reported historical period for \$\{subject\}\./, 'within-window copy is anchored to the subject without prediction');
assert.match(source, /Right now is outside the most commonly reported period for \$\{subject\}\./, 'outside-window copy avoids live-clear claims');
...
assert.match(source, /outside_common_window/, 'outside relationship is explicit for audit fixtures');
