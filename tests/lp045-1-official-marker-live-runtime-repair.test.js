...
assert(app.includes('const id = publication.markerModelIdentity || String(gridlyLp045OfficialMarkerIdentity(record, `drivetexas-${index}`) || "");'), 'marker creation uses the governed publication identity with the shared official identity fallback');
