...
assert.strictEqual(audit.previewActionAvailable, true);
assert.strictEqual(audit.previewCompleteFlowAvailable, true);
assert.strictEqual(audit.prototypeResultPreviewed, false);
assert.strictEqual(audit.prototypeResultSaved, false);
assert.strictEqual(audit.previewActionProductionWrites, 0);
assert.strictEqual(audit.previewActionStateMutations, 0);
...
assert.strictEqual(retryState.resolverResult, null, 'Try another ZIP clears resolver state');
assert.strictEqual(retryState.prototypeResult, null, 'Try another ZIP clears prototype result state');
