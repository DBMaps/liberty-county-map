...
assert.match(edge, /GRIDLY_AUTHORITATIVE_RURAL_API_KEY/);
...
assert.match(edge, /supportedTexasCounties = new Set\(\[/);
...
(async () => {
  ...
  const certified = await window.gridlyLp104VisibleRegionalRuralAddressCertification({ cases, protectedLibertyValidation: true });
  for (const field of ['providerBoundaryPreserved', 'allCountyArchitecturePass', 'ruralHouseNumberSafetyPass', 'ruralRoadIdentityPass',
    'ruralPrecisionPass', 'countyContainmentPass', 'rejectedMismatchAbsent', 'routePreviewAgreement', 'privateDiagnosticsRedacted',
    'fallbackOrderingPass', 'ordinaryConsumerSearchPass', 'failedChecks', 'safeToMerge']) assert.notEqual(certified[field], undefined);
  ...
  const serialized = JSON.stringify(await window.gridlyLp104RegionalRuralAddressAudit({ cases, protectedLibertyValidation: true }));
