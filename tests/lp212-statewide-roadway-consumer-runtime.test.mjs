...
const sequence=['Liberty','Anderson','Bexar','Dallas','Harris','Dallas','Bexar','FAILED_ANDERSON','Chambers','Liberty'];
...
function validRoadIdentity(properties={}){return [properties.name,properties.ref,properties.highway,properties.FULLNAME].map(value=>String(value||'').trim().replace(/\s+/g,' ')).find(value=>value.length>=3&&!/^[-,.\s]+$/.test(value)&&!invalidRoadNames.has(value.toLowerCase()));}
...
async function makeHarness(){
  ...
  ${productionFunction('getRoadwayFeatureNameCandidates')} ${productionFunction('buildResolveNearestRoadNameIndexKey')} ${productionFunction('resolveNearestRoadName')}
...
test('LP212 actual consumer runtime certifies statewide architecture transitions',async()=>{
  ...
  await mkdir(new URL('reports/lp212/',root),{recursive:true});await writeFile(new URL('reports/lp212/lp212-consumer-runtime-certification.json',root),`${JSON.stringify(report,null,2)}\n`);
    ...
    const report={milestone:'LP212',mode:'CONSUMER_RUNTIME_CERTIFICATION',productionManifest:{countyCount:Object.keys(manifest.counties).length,sha256:createHash('sha256').update(manifestBytes).digest('hex'),identityPass:true},countyMatrix:matrix,transitionSequence:sequence,executionFailure:{...failureContext,errorName:error?.name||'Error',errorMessage:error?.message||String(error)},protectedSystemsMutated:false,productionDefectsFound:[{id:'LP212_ROADWAY_IDENTITY_SCHEMA_COMPATIBILITY',county:'Anderson',observedGeometryCount:4752,existingResolverFieldsPresent:0,fallbackField:'FULLNAME',status:'REPAIRED'}],finalDecision:'EXECUTION_FAILED'};
    await mkdir(new URL('reports/lp212/',root),{recursive:true});await writeFile(new URL('reports/lp212/lp212-consumer-runtime-certification.json',root),`${JSON.stringify(report,null,2)}\n`);
    throw error;
  }
});
