...
test("portrait restores the original destination command Search entry", () => {
  ...
  assert.match(html, /<article[^>]*id="mobileDestinationCommandPanel"[^>]*class="[^"]*\bmobile-destination-command\b[^"]*"[^>]*>[\s\S]*?mobile-destination-command-actions[\s\S]*?id="mobileDestinationCommandBtn"[\s\S]*?aria-controls="gridlySearchShell"/);
