...
test('Android Gradle wrapper is a complete tracked Gradle 8.14.4 authority', () => {
  const wrapperFiles = [
    'android/gradlew',
    'android/gradlew.bat',
  ...
  for (const path of wrapperFiles) {
    assert.ok(existsSync(path), `clean checkout wrapper input ${path} must exist`);
    assert.ok(tracked.has(path), `clean checkout wrapper input ${path} must be tracked`);
  }

  const properties = text(wrapperFiles[3]);
  assert.match(properties, /^distributionUrl=https\\:\/\/services\.gradle\.org\/distributions\/gradle-8\.14\.4-bin\.zip$/m);
  assert.match(text(wrapperFiles[0]), /-jar "\$APP_HOME\/gradle\/wrapper\/gradle-wrapper\.jar"/);
  assert.match(text(wrapperFiles[1]), /-jar "%APP_HOME%\\gradle\\wrapper\\gradle-wrapper\.jar"/);
  assert.equal(
    createHash('sha256').update(readFileSync(wrapperFiles[2])).digest('hex'),
    '7d3a4ac4de1c32b59bc6a4eb8ecb8e612ccd0cf1ae1e99f66902da64df296172',
    'wrapper JAR must be the artifact generated by Gradle 8.14.4'
  );

  assert.equal(tracked.has('android/local.properties'), false, 'machine SDK path must not be tracked');
  execFileSync('git', ['check-ignore', '--quiet', 'android/local.properties']);
});
