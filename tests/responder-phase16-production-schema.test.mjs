// Phase 16 production-candidate validation in an explicit disposable localhost cluster.
...
const {RESPONDER_PHASE16_PGBIN:bin,RESPONDER_PHASE16_PGPORT:port,
  RESPONDER_PHASE16_PGUSER:owner,RESPONDER_PHASE16_PGDATA:data}=process.env;
if(!bin||!/^[0-9]{4,5}$/.test(port||'')||!data||!fs.existsSync(data)
  ||!path.basename(data).startsWith('gridly-responder-phase16-')
  ||path.relative(os.tmpdir(),path.resolve(data)).startsWith('..'))
  throw new Error('Explicit disposable Phase 16 localhost PostgreSQL configuration required');
...
const database=`gridly_responder_p16_${randomUUID().replaceAll('-','').slice(0,18)}`;
...
try{
  ...
  apply('phase16_test_bootstrap.sql');
  apply('phase16_production_candidate.sql');
  ...
  const rollbackDb=`gridly_responder_p16_rb_${randomUUID().replaceAll('-','').slice(0,16)}`;
  psql('postgres',['-c',`CREATE DATABASE ${rollbackDb}`]);
  try {
    psql(rollbackDb,['-f',path.join(root,'db','responder-local','phase16_test_bootstrap.sql')]);
    psql(rollbackDb,['-f',path.join(root,'db','responder-local','phase16_production_candidate.sql')]);
    psql(rollbackDb,['-f',path.join(root,'db','responder-local','phase16_production_candidate_rollback.sql')]);
  ...
  console.log(`PHASE16_SECURITY_VECTORS passed=${passed} failed=${failed} total=${passed+failed}`);
  console.log(`PHASE16_DATABASE_CHECKS passed=${dbPassed} failed=${dbFailed} total=${dbPassed+dbFailed}`);
