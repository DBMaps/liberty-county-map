...
async function request(auth, path, init={}, hooks={}) {
  const controller=new AbortController(); const timer=setTimeout(()=>controller.abort(),hooks.timeoutMs??15000);
  const fetchImpl=hooks.fetchImpl||fetch;
  try { for(let n=0;n<(hooks.attempts??3);n++){ const r=await fetchImpl(`${auth.url}/storage/v1/${path}`,{...init,headers:{...credentialHeaders(auth.key),...init.headers},signal:controller.signal}); if(r.status<500||n===(hooks.attempts??3)-1)return r; await new Promise(x=>setTimeout(x,200*(n+1))); } } finally {clearTimeout(timer);}
}
...
const safeDiagnostic=(status,body)=>redact(`Storage download failed (${status})${errorEvidence(body)?`: ${errorEvidence(body)}`:''}`).slice(0,320);
