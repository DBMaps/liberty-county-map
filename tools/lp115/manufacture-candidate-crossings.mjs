...
const DEFAULT_INDEX = join(ROOT, 'evidence/wave3a2-crossing-package-manufacture/reconciliation-index.json');
...
export function buildReconciliationLookup(index) {
  if (!index || !Array.isArray(index.entries)) throw new Error('reconciliation index has no entries');
  const lookup = new Map();
  for (const entry of index.entries) {
    const id = String(entry?.crossingId || '').trim();
    if (!id) throw new Error('reconciliation index entry has no crossingId');
    if (lookup.has(id)) throw new Error(`duplicate index crossing: ${id}`);
    lookup.set(id, entry);
  }
  return lookup;
}

export function selectIndexedFeatures(source, county, lookup) {
  const selected = [];
  for (const feature of source.features) {
    const id = String(feature?.properties?.CROSSING || '').trim();
    if (!id) throw new Error('FRA crossing record has no CROSSING identity');
    const entry = lookup.get(id);
    if (!entry) throw new Error(`missing index crossing: ${id}`);
    if (entry.crossingId !== id) throw new Error(`FRA crossing ID mismatch: ${id}`);
    if (entry.resolution === 'OUTSIDE_TEXAS_BORDER_REVIEW') continue;
    if (!entry.gridlyCountyFips) throw new Error(`missing Gridly FIPS for packageable row: ${id}`);
    if (!['SOURCE_AND_GEOGRAPHY_AGREE', 'CLEAR_GEOGRAPHIC_REASSIGNMENT', 'BORDER_TOLERANCE_REVIEW'].includes(entry.resolution)) throw new Error(`conflicting reconciliation evidence: ${id}`);
    if (entry.gridlyCountyFips === county.fips) selected.push(feature);
  }
  return selected;
}

async function manufactureCounty(county, source, sourceEvidence, options, lookup) {
  ...
  const selected = selectIndexedFeatures(source, county, lookup);
  ...
  const base = { schemaVersion: 'gridly-lp115-crossing-candidate-v2', county: `${county.countyName} County`, countyId: county.countyId, fips: county.fips, authoritativeSource: 'FRA Texas statewide crossing GeoJSON', source: sourceEvidence, sourceQueryCompleted: true, sourceFilter: { property: 'reconciliation-index.gridlyCountyFips', value: county.fips }, sourceRecordsSelected: selected.length, productionAuthorized: false, activated: false, uploadEnabled: false, deploymentEnabled: false };
  if (!selected.length) {
  ...
  const productionPackage = { type: 'FeatureCollection', features: productionFeatures };
...
export async function manufacture(options) {
  ...
  let index;
  try { index = options.index || cleanJson(await readFile(resolve(options.indexPath || DEFAULT_INDEX), 'utf8')); }
  catch (error) { throw new Error(`reconciliation index unavailable: ${error.message}`); }
  const lookup = buildReconciliationLookup(index);
  if (lookup.size !== source.features.length) throw new Error(`reconciliation join completeness failure: ${lookup.size} index entries for ${source.features.length} FRA rows`);
  const inventoryFips = new Set(inventory.counties.map(x => x.fips));
  for (const entry of lookup.values()) if (entry.gridlyCountyFips && !inventoryFips.has(entry.gridlyCountyFips)) throw new Error(`unknown Gridly county: ${entry.gridlyCountyFips}`);
  const evidence = { identity: portable(sourcePath), sizeBytes: Buffer.byteLength(sourceBody), sha256: sha(sourceBody) }; const results = [];
  for (const county of counties) { try { results.push(await manufactureCounty(county, source, evidence, options, lookup)); } catch (error) { const failed = { county: `${county.countyName} County`, fips: county.fips, status: 'FAILED', source: evidence, productionAuthorized: false, activated: false, blockingReasons: [error.message] }; await atomicJson(join(reports, county.fips, 'checkpoint.json'), failed); results.push(failed); } }
...
export function parseArguments(argv) { const out = { candidate: false }; for (let i = 0; i < argv.length; i++) { const arg = argv[i]; if (['--candidate', '--resume'].includes(arg)) out[arg.slice(2)] = true; else if (['--fips', '--reports', '--source', '--index'].includes(arg)) { if (!argv[i + 1]) throw new Error(`${arg} requires a value`); out[arg.slice(2).replace('index', 'indexPath')] = argv[++i]; } else if (arg === '--help' || arg === '-h') out.help = true; else throw new Error(`Unknown option: ${arg}`); } if (!out.help && !out.fips) throw new Error('--fips is required'); if (!out.help && !out.candidate) throw new Error('--candidate is required; LP115 cannot activate production'); return out; }
