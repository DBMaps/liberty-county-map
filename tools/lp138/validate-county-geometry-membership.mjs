...
export function membershipSha256(members) {
...
export function validateMembershipContract(contract, { registryMembers, packageMembers, expectedContractKind } = {}) {
  ...
  for (const member of members) {
    ...
    member.gates.forEach((gate, index) => {
      if (gate.gate !== index + 1 || !['PASS', 'BLOCKED', 'NOT_EVALUATED'].includes(gate.status) || !gate.evidenceRef) {
        fail(`invalid sequential Gate ${index + 1} evidence for ${member.countyId}`);
      }
    });
  }
  ...
  if (PERMISSIONS.some(name => typeof p[name]?.authorized !== 'boolean' || !p[name]?.authorityRef)) {
    fail('each separate permission requires authorized and authorityRef');
  }
  if (p.generateRuntimePackage.authorized && contract.approval.status !== 'APPROVED_FOR_PACKAGE_GENERATION') fail('package generation lacks exact approval state');
  if (p.deploy.authorized && contract.approval.status !== 'APPROVED_FOR_DEPLOYMENT') fail('deployment lacks exact approval state');
  if (p.activateRuntime.authorized) {
