...
if (process.argv[1] && import.meta.url === `file://${process.argv[1]}`) { try { const result = process.argv.includes('--write') ? writeAll() : verify(); console.log(json(result)); if (result.status !== 'PASS') process.exitCode = 1; } catch (e) { console.error(e.message); process.exitCode = 1; } }
