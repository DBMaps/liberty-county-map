...
export function certify(root = ROOT) {
  ...
  return {
    ...
    'secret-presence-certification.json': {...common, valueFieldsPermitted:false, secretValuesRead:0, items:requirements.filter(x=>x[4]==='HIGH_SECRET').map(x=>({identifier:x[0],status:recordStatus(evidence,`SECRET:${x[0]}`),evidence:'NAME_AND_STATUS_ONLY'}))},
    ...
    'owner-evidence-requirements.json': {...common,status:evidence||sqlEditorEvidence?'PARTIAL_EVIDENCE_INGESTED':'OWNER_EXECUTION_REQUIRED',workflow:'npm run capture:lp169:remaining-evidence -- -ProjectRef <ref> -Repository <owner/repo>',sqlEditorEvidence:sqlEditorEvidence?sqlEditorEvidence.evidenceStatus:'OWNER_SQL_EDITOR_EXPORT_INGESTION_REQUIRED',schema:{schemaVersion:2,generatedByWorkflow:true,records:'REDACTED_NAME_STATUS_AND_HASH_METADATA_ONLY'},requirements:['SUPABASE_PROJECT_IDENTITY','FUNCTION:gridly-geocode','SECRET_NAME_INVENTORIES','EXACT_STORAGE_OBJECT_INVENTORY','STORAGE_POLICY_BEHAVIOR','PRODUCTION_ORIGIN_ATTESTATION','RUNTIME_ALIGNMENT','APP_DISTRIBUTION_ATTESTATION'],rawCommandOutputAccepted:false},
    'configuration-blockers.json': {...common,blockers},
    'certification-summary.json': {...common,overallClassification:evidence?'REMAINING_PRODUCTION_CONFIGURATION_EVIDENCE_RECONCILED':'REMAINING_PRODUCTION_CONFIGURATION_EVIDENCE_READY',decisions,deploymentAuthorized:false,activationAuthorized:false,appDistributionAuthorized:false,publicLaunchAuthorized:false,productionWrites:0,deployments:0,activations:0,secretValuesRead:0,relationshipToLp168:'Configuration evidence does not authorize operations; a future LP167 reassessment is required.'},
