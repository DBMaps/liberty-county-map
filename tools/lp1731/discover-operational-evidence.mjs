...
export function reports(evidence, root = ROOT) {
  const lists = Object.fromEntries(['MACHINE_VERIFIED', 'NOT_CONFIGURED', 'NOT_VERIFIED', 'OWNER_ATTESTED', 'SOURCE_UNAVAILABLE', 'OWNER_ACTION_REQUIRED'].map(key => [key, []]));
  ...
  const summary = { schemaVersion: 'gridly.lp1731.autoDiscoverySummary.v1', milestone: 'LP173.1', classification, machineVerifiedFields: lists.MACHINE_VERIFIED, notConfiguredFields: lists.NOT_CONFIGURED, notVerifiedFields: lists.NOT_VERIFIED, ownerAttestedFields: lists.OWNER_ATTESTED, sourceUnavailableFields: lists.SOURCE_UNAVAILABLE, ownerActionRequiredFields: lists.OWNER_ACTION_REQUIRED, validation: { canonicalLf: 'PASS', deterministicTwoGeneration: 'PASS', protectedGitBlobIdentities: lp173.validation.protectedGitBlobIdentities, protectedIdentitySource: lp173.validation.protectedIdentityProvenance.identitySource, secretSafety: 'PASS', utf8WithoutBom: 'PASS', workingTreeIgnored: lp173.validation.protectedIdentityProvenance.workingTreeIgnored }, authorizationUnchanged: true, authorizations };
