...
export function build(){
 ...
 const validPackages=packages.filter(x=>x.identityValid),valid=new Set(validPackages.map(x=>x.fips)), missing=packages.filter(x=>!x.present), invalid=packages.filter(x=>x.present&&!x.identityValid),placeGeometry=discoverPlaceGeometry();
 ...
 const classification='NOT_READY_OWNER_INPUT_REQUIRED';
 return {schemaVersion:'gridly.lp200.statewide-governed-place-populated-core-signal-certification.v1',milestone:'LP200',classification,scope:{certificationOnly:true,runtimeActivation:false,identityMutation:false,countyMembershipMutation:false,zipMutation:false,cameraRegistryMutation:false},generatedFrom:Object.fromEntries(['projection','presentation','geometry','overrides','manifest','source','license','reconciliation'].map(k=>[k,{path:P[k],sha256:hash(P[k])}])),sourceInventory:[
 ...
 authorizationGate:{classification:'UNRESOLVED_GOVERNANCE_CLASSIFICATION_NOT_A_KNOWN_PROHIBITION',internalCertificationOnlyComputation:{knownProhibited:false,status:'OWNER_GOVERNANCE_CLASSIFICATION_REQUIRED_BEFORE_EXECUTION'},commitDerivedPopulatedCoreCoordinates:{knownProhibited:false,status:'DERIVATIVE_AUTHORIZATION_UNRESOLVED_DO_NOT_COMMIT_PENDING_CLASSIFICATION'},redistributeRawAddressRecords:{permitted:false,status:'NOT_AUTHORIZED'},rawAddressStorage:{permitted:true,status:'GOVERNED_EXISTING'},finding:'Raw redistribution, internal computation, and committing derived coordinates are separate decisions; LP200 does not convert an unresolved derivative classification into a claim that technical derivation is impossible.'},
 executionReadiness:{technicalInputsReady:valid.size===254&&placeGeometry.preflight==='PASS',fullStatewideAddressCoverageReady:valid.size===254,calibrationCoverage,calibrationPlacesReady:calibrationCoverage.filter(x=>x.complete).map(x=>x.label),calibrationPlacesBlocked:calibrationCoverage.filter(x=>!x.complete).map(x=>x.label),polygonReady:placeGeometry.preflight==='PASS',authorizationForInternalComputation:'UNRESOLVED_OWNER_CLASSIFICATION_REQUIRED',realProcessingCanRun:false,reason:'Current checkout lacks complete calibration address coverage and the governed polygon bytes; internal-use governance classification also remains required.'},
