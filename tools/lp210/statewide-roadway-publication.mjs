...
export async function verifyPortable(){const {counties}=await buildPlan(),s=await json(join(REPORTS,'statewide-roadway-package-publication.json')),m=await json(join(REPORTS,'statewide-roadway-remote-certification-manifest.json'));if(m.counties.length!==226||s.publicationCohortCount!==226||s.runtimeActivations!==0)throw Error('Invalid LP210 portable evidence');if(s.readiness==='READY_FOR_STATEWIDE_ROADWAY_RUNTIME_ACTIVATION'&&(!m.counties.every(c=>c.remoteCertificationStatus==='PASS')||s.remoteCertifiedCountyCount!==226))throw Error('False LP210 readiness claim');return s;}
