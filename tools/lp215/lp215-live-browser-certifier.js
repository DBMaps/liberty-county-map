(function installGridlyLp215LiveCertifier(global) {
  ...
  const SEED_SOURCE_TIMEOUT_MS = 15000;
  ...
  function seedSettlement(row, s) {
    const railEmpty = row.railManifestStatus === 'ACTIVE_EMPTY' && row.railGovernedCount === 0;
    const railCount = Number(s.rail.runtimeCrossingInventoryCount);
    const renderCalls = Number(s.rail.renderCrossingsCallCount || 0);
    const railSourceCounty = s.context.runtimeInventoryCounty || s.rail.runtimeInventoryCounty || s.rail.sourceCountyId || s.rail.activeCountyId || null;
    const roadwayState = String(s.road.lifecycleState || s.road.loadState || s.road.status || '').toUpperCase();
    const driveTerminal = TERMINAL.has(s.drive);
    const officialCount = s.shared.activeOfficialRoadwayCount ?? s.official.representedRecordCount;
    const eligible = s.alertsSnapshot?.activeLocalizedAlertCount ?? s.shared.activeIssueCount;
    const conditions = {
      selectedCommunityMatchesExpected: s.context.awarenessAreaKey === row.canonicalKey,
      activeCountyMatchesExpected: s.context.activeCountyId === row.countyId,
      cameraSettled: near(s.map.center,row.semanticCameraTarget) && Number(s.map.zoom) === Number(row.semanticCameraTarget?.zoom),
      roadwayCountyStateSettled: s.road.loadedRoadwayCounty === row.countyId,
      roadwayLoadTerminal: s.road.activeCountyPackageLoaded === true || TERMINAL.has(roadwayState),
      driveTexasLifecycleTerminal: driveTerminal,
      officialRoadwayConsumerSettled: Number.isInteger(officialCount) || driveTerminal,
      alertsConsumerSettled: (s.alertsSnapshot != null && Number.isInteger(Number(eligible))) || driveTerminal,
      railSourceCountySettled: railSourceCounty === row.countyId,
      railInventoryTerminal: railEmpty ? railCount === 0 && renderCalls > 0 : s.context.inventoryHydrationCompleted === true,
      railPresentationTerminal: railEmpty ? railCount === 0 && renderCalls > 0 : s.context.inventoryHydrationCompleted === true && renderCalls > 0,
      staleStatePredecessorCleanupComplete: true
    };
    const unsatisfied = Object.entries(conditions).filter(([,pass])=>!pass).map(([name])=>name);
    return { settled:unsatisfied.length===0, conditions, unsatisfied, evidence:{driveState:s.drive,railSourceCounty,railInventoryCount:railCount,renderCrossingsCallCount:renderCalls,filterApplyCallCount:Number(s.rail.filterApplyCallCount||0)} };
  }
