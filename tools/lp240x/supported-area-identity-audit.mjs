...
const hou=vm.runInNewContext(expr('GRIDLY_LP035_HOUSTON_REGION_MODEL'));for(let x of hou)if(!starters.some(a=>a.key===x.id))starters.push({key:x.id,label:x.label,storageValue:`Houston — ${x.label}`,countyId:'harris-tx',lat:x.lat,lng:x.lng,radiusMiles:5,houstonRegion:true,source:'LP035.1 Houston regional awareness model'});
const sa=vm.runInNewContext(expr('GRIDLY_LP194_SAN_ANTONIO_REGION_MODEL'));for(let x of sa)if(!starters.some(a=>a.key===x.id))starters.push({key:x.id,label:x.label,storageValue:`San Antonio — ${x.label}`,countyId:x.countyId,lat:x.lat,lng:x.lng,radiusMiles:null,sanAntonioRegion:true,source:'LP194 certified LP193 geometry activation'});
