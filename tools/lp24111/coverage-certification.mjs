...
export function executeCoverage({directory=path.join(root,'owner-local/lp24111'),executable=process.env.DUCKDB||'duckdb',expectedStandaloneCount=391772}={}){
 ...
 const countyAgg=run('MEASURE_COUNTIES',`SELECT county_fips,count(*)::BIGINT standalone_poi_count,${categoryColumns},count(*) FILTER(WHERE trim(brand_text)<>'')::BIGINT brand_bearing_count FROM poi GROUP BY 1 ORDER BY 1`);
 ...
 const radiusSelect=RADII_MILES.flatMap(m=>[`count(p.id) FILTER(WHERE d<=${m})::BIGINT within_${m}`,...CORE_CATEGORIES.map(c=>`count(p.id) FILTER(WHERE d<=${m} AND p.gridly_category=${quote(c)})::BIGINT ${c.toLowerCase()}_${m}`)]).join(',');
  const measureAnchors=(stage,file,out)=>run(stage,`COPY (WITH anchors AS (SELECT * FROM read_json_auto(${quote(file)})), pairs AS (SELECT a.*,p.id,p.gridly_category,${distance} d FROM anchors a LEFT JOIN poi p ON p.latitude BETWEEN a.latitude-0.37 AND a.latitude+0.37 AND p.longitude BETWEEN a.longitude-0.5 AND a.longitude+0.5) SELECT * EXCLUDE(id,gridly_category,d),${radiusSelect},${RADII_MILES.map(m=>`(floor(latitude+${m}/69.0)-floor(latitude-${m}/69.0)+1)*(floor(longitude+${m}/(69*cos(radians(latitude))))-floor(longitude-${m}/(69*cos(radians(latitude))))+1) fanout_${m}`).join(',')} FROM pairs GROUP BY ALL ORDER BY 1) TO ${quote(out)} (FORMAT PARQUET,COMPRESSION ZSTD)`,{json:false});
 ...
 const accessibility=run('MEASURE_CATEGORY_ACCESSIBILITY',`WITH anchors AS (SELECT * FROM read_json_auto(${quote(path.join(directory,'coverage-all-anchors.json'))})), pairs AS (SELECT a.stableGovernedIdentity,a.communityId,p.gridly_category,${distance} d FROM anchors a CROSS JOIN poi p WHERE p.gridly_category IN (${CORE_CATEGORIES.map(quote)}) AND p.latitude BETWEEN a.latitude-1 AND a.latitude+1 AND p.longitude BETWEEN a.longitude-1.5 AND a.longitude+1.5) SELECT coalesce(stableGovernedIdentity,communityId) governed_identity,gridly_category,min(d) nearest_miles,${RADII_MILES.map(m=>`count(*) FILTER(WHERE d<=${m})::BIGINT within_${m}`).join(',')} FROM pairs GROUP BY 1,2 ORDER BY 1,2`);
 const brands=run('MEASURE_BRANDS',`SELECT brand_text brand,count(*)::BIGINT standalone_record_count,count(DISTINCT county_fips)::BIGINT counties_represented FROM poi WHERE trim(brand_text)<>'' GROUP BY 1 ORDER BY 2 DESC,1`);run('MEASURE_BRANDS',`COPY (SELECT brand_text brand,count(*)::BIGINT standalone_record_count,count(DISTINCT county_fips)::BIGINT counties_represented FROM poi WHERE trim(brand_text)<>'' GROUP BY 1 ORDER BY 2 DESC,1) TO ${quote(path.join(directory,'brand-coverage.parquet'))} (FORMAT PARQUET,COMPRESSION ZSTD)`,{json:false});
 const metadataRows=run('MEASURE_METADATA',`SELECT CASE WHEN trim(locality)='' AND trim(address_text)='' THEN 'SPATIAL_METADATA_INCOMPLETE' WHEN regexp_matches(upper(address_text),'(,| )MO( |,|$)|MISSOURI|65101') THEN 'SPATIAL_METADATA_CONFLICT' ELSE 'SPATIAL_METADATA_CONSISTENT' END classification,count(*)::BIGINT count FROM poi GROUP BY 1 ORDER BY 1`);run('MEASURE_METADATA',`COPY (SELECT id,display_name,county_fips,locality,address_text,CASE WHEN trim(locality)='' AND trim(address_text)='' THEN 'SPATIAL_METADATA_INCOMPLETE' WHEN regexp_matches(upper(address_text),'(,| )MO( |,|$)|MISSOURI|65101') THEN 'SPATIAL_METADATA_CONFLICT' ELSE 'SPATIAL_METADATA_CONSISTENT' END classification FROM poi) TO ${quote(path.join(directory,'metadata-conflicts.parquet'))} (FORMAT PARQUET,COMPRESSION ZSTD)`,{json:false});
 ...
 const envelope={schemaVersion:'gridly.lp24111.measured-coverage.v1',generatedAt:new Date().toISOString(),input:{path:input,rows:count,sha256:before,schema},reports};const gate=validateEnvelope(envelope);reports['certification.json'].evidenceCompletenessGate=gate.gates;reports['certification.json'].executiveResult=gate.passed?'PHASE_D4_MEASURED_STATEWIDE_COVERAGE_AND_QUALITY_CERTIFIED':'PHASE_D4_MEASUREMENT_INCOMPLETE';
