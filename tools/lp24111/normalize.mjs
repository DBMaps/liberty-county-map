...
export function execute(options={}){
 ...
 const byClass=Object.fromEntries(classes.map(x=>[x.class,Number(x.count)])),eligibleCount=byClass.GRIDLY_ELIGIBLE_DESTINATION??0;
 const medical=queryOne(`SELECT count(*) FILTER(WHERE regexp_matches(searchable,'hospital|emergency_room|emergency_department|urgent_care|pharmacy|veterinar|animal_hospital|animal_clinic|pet_wellness')) raw_candidates,count(*) FILTER(WHERE regexp_matches(searchable,'veterinar|animal_hospital|animal_clinic|pet_wellness')) veterinary_exclusions,count(*) FILTER(WHERE gridly_category='HOSPITAL') hospital,count(*) FILTER(WHERE gridly_category='EMERGENCY_CARE') emergency,count(*) FILTER(WHERE gridly_category='URGENT_CARE') urgent,count(*) FILTER(WHERE gridly_category='PHARMACY') pharmacy FROM read_parquet(${normalized})`,options);
 const countyRows=duck(`SELECT county_fips,count(*) rawUniqueRich,count(*) FILTER(WHERE eligibility_class='GRIDLY_ELIGIBLE_DESTINATION') eligibleDestination,count(*) FILTER(WHERE eligibility_class='GRIDLY_NON_DESTINATION') nonDestination,count(*) FILTER(WHERE eligibility_class='GRIDLY_EXCLUDED') excluded,count(*) FILTER(WHERE eligibility_class='GRIDLY_REVIEW_REQUIRED') reviewRequired FROM read_parquet(${normalized}) GROUP BY 1 ORDER BY 1`,options);if(countyRows.length!==254)throw Error(`Normalized county coverage is ${countyRows.length}, expected 254`);
 const packageRows=duck(`SELECT regexp_extract(filename,'(tx-[0-9]{2}-[0-9]{3})',1) shardId,count(*) rawRows FROM read_parquet(${glob},filename=true) GROUP BY 1 ORDER BY 1`,options);
