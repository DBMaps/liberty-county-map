...
export function measurementQueries({normalized,v2}){
 ...
 return {
  ...
  primaryCategory:`SELECT coalesce(json_extract_string(to_json(categories),'$.primary'),'(null)') AS category_name, count(*)::BIGINT AS record_count ${review} GROUP BY 1 ORDER BY record_count DESC, category_name LIMIT 100`,
  hierarchy:`SELECT coalesce(to_json(taxonomy),'(null)') AS hierarchy_value, count(*)::BIGINT AS record_count ${review} GROUP BY 1 ORDER BY record_count DESC, hierarchy_value LIMIT 100`,
  nameCategory:`SELECT coalesce(json_extract_string(to_json(names),'$.primary'),'(null)') AS place_name, coalesce(basic_category,'(null)') AS category_name, count(*)::BIGINT AS record_count ${review} GROUP BY 1,2 ORDER BY record_count DESC, place_name, category_name LIMIT 100`,
...
export function execute({directory=localDefault,query=duck}={}){
 ...
 try{
  ...
  for(const {shard_id:id} of shardRows){const [,y,x]=id.split('-'),raw=path.join(packageDir,`${id}.ndjson`),gz=`${raw}.gz`;run('COMPACT_PACKAGE_MATERIALIZATION',`COPY (SELECT id AS id, coalesce(json_extract_string(to_json(names),'$.primary'),'') AS displayName, lower(coalesce(json_extract_string(to_json(names),'$.primary'),'')) AS normalizedName, json_extract_string(to_json(brand),'$.names.primary') AS brand, gridly_category AS gridlyCategory, latitude AS latitude, longitude AS longitude, county_fips AS countyFips, json_extract_string(to_json(addresses),'$[0].locality') AS locality, operating_status AS operatingStatus FROM read_parquet(${v2}) WHERE eligibility_class='GRIDLY_ELIGIBLE_DESTINATION' AND floor(latitude)=${Number(y)} AND floor(longitude)=-${Number(x)} ORDER BY id) TO ${quote(raw)} (FORMAT JSON,ARRAY false);`);const bytes=fs.readFileSync(raw),compressed=zlib.gzipSync(bytes,{level:9});fs.writeFileSync(gz,compressed);measurements.push({shardId:id,eligibleRows:bytes.length?bytes.toString().trim().split('\n').length:0,rawBytes:bytes.length,compressedBytes:compressed.length});}
