...
async function execute(plan) {
  ...
  fs.mkdirSync(path.dirname(localProgress), {recursive:true});
  const progress = fs.existsSync(localProgress) ? JSON.parse(fs.readFileSync(localProgress,'utf8')) : {rows:[]};
  const done = new Set(progress.rows.map(x => x.countyFips));
  for (const row of plan.fixtures) {
    if (done.has(row.countyFips)) continue;
    const started = Date.now();
    const response = await fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json',Accept:'application/json',apikey:publicKey,Authorization:`Bearer ${publicKey}`},body:JSON.stringify({intent:'address',query:row.privacySafeSeedQuery,limit:3,requestMode:'explicit_search',requestId:`lp2416-${row.countyFips}`})});
    if ([401,403,429].includes(response.status) || response.status >= 500) throw Error(`safe stop: provider HTTP ${response.status} at ${row.countyFips}`);
    ...
    fs.writeFileSync(localProgress,`${JSON.stringify(progress,null,2)}\n`);
  }
}
...
if (process.argv.includes('--execute')) await execute(plan);
