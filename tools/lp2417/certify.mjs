...
const inventory={schemaVersion:2,evidenceType:'DETERMINISTIC_SOURCE_AND_RENDERED_DOM_AUDIT',generatedAt:'2026-08-27',ownerCommand:'window.gridlyLP2417KeyboardAcceptance()',summary:{nativeInteractiveElements:tags.length,byElement:count,genericFocusableElements:generic.length},genericFocusableControls:generic,contracts:{accessibleNames:'Rendered native text, associated labels, aria-label and aria-labelledby are projected without activation',keyboard:'Rendered browser tab-order projection plus native activation and bounded custom key-handler source contracts',hiddenState:'Computed visibility plus hidden, aria-hidden, inert and open-modal leakage are inspected',passive:'No click, focus, submit, event dispatch, network, or storage operation'},ownerRuntimeEvidence:{classification:'OWNER_RUNTIME_PASS',deterministicPass:true,focusableCount:18,essentialControlCount:4,exceptionCount:0},limitations:['DOM order is a deterministic projection, not a claim of exhaustive Narrator certification. Narrator semantic entry and the bounded owner runtime lifecycle evidence are accepted; no further Windows AT action remains.']};
const responsive={schemaVersion:2,evidenceType:'DETERMINISTIC_SOURCE_AUDIT',viewports:['desktop','tablet/narrow desktop','mobile portrait','mobile landscape','200% zoom pressure'],certified:{viewportZoomEnabled:checks.viewportAllowsZoom,safeAreaTokens:checks.safeAreas,dynamicViewportUnits:checks.dynamicViewport,reducedMotion:checks.reducedMotion,scrollableOverlays:/overflow(-y)?\s*:\s*(auto|scroll)/.test(css)},ownerEvidenceRequired:['One reduced iPhone Safari notch/mobile-chrome, increased-text, and rotation smoke','One reduced Android Chrome touch, increased-text, and rotation smoke'],notRequired:['A separate desktop 200% owner journey','A repeated Windows Narrator journey']};
