...
const out='C:/GitHub/liberty-county-map/.artifacts/lp24447-marker-system-review';
...
try {
 ...
 const evidence={phase:'LP244.47',startingCommit:'9c9e082d',synthetic:true,backendWrites:false,reportingActivated:false,tileAccess:'Only GET public map tiles; all backend requests blocked/stubbed',maps:[],mobile:[],assets:entries,errors};
 async function board(name,title,items,width=1440,before=false,official=false){
  ...
  await page.evaluate(({title,items,before,official,inventory})=>{
   ...
   const utility=items.length===31?'<div style="margin-top:22px;padding:18px;background:#102b40;color:white;border-radius:14px"><b>Non-hazard map objects — existing semantics and dimensions preserved</b><div style="display:flex;flex-wrap:wrap;gap:25px;margin-top:16px;align-items:center"><div><span class="gridly-user-location-awareness-dot" style="position:relative;display:block;width:30px;height:30px"><span class="gridly-user-location-awareness-dot__ring"></span><span class="gridly-user-location-awareness-dot__core"></span></span>32. User location · 30 px</div><div><div class="gridly-destination-signature-marker" style="width:32px;height:32px"><span class="gridly-destination-ring"></span><span class="gridly-destination-core"></span></div>33. Destination · 32 px</div><div><span class="gridly-route-endpoint gridly-route-endpoint-origin" style="display:block;width:12px;height:12px"></span>34. Route origin · 12 px</div><div><span class="gridly-route-endpoint gridly-route-endpoint-destination" style="display:block;width:18px;height:18px"></span>35. Route destination · 18 px</div><div><span style="display:block;width:8px;height:8px;border:1px solid #8fb6ff;border-radius:50%;background:#8fb6ff4d"></span>36. Placement preview · radius 4</div><div>Suppressed: awareness map label<br>Area panel owns identity</div></div></div>':'';
   el.innerHTML='<div style="color:#12616b;font-size:12px;letter-spacing:2px;font-weight:800">GRIDLY · LP244.47 · OWNER REVIEW</div><h1 style="font-size:30px;line-height:1.15;margin:10px 0">'+title+'</h1><p>Actual implemented SVG assets at 64 × 64 CSS pixels. Symbols identify conditions; source attribution stays separate.</p><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(145px,1fr));gap:12px">'+cards+'</div>'+utility+'<p style="margin-bottom:0;font-size:12px">Synthetic local preview • no submissions or backend writes • visual approval pending</p>';
 ...
 await board('complete-marker-library','The complete Gridly marker library',entries);
 ...
 await board('complete-marker-library-mobile','64 px mobile library',entries,390);
 await board('winter-marker-final','Seven distinct winter conditions',entries.filter(e=>e.family==='winter'));
 await board('community-vs-official-road-state','Observation and official road status',entries.filter(e=>['road_blocked','road_impassable','road_closed'].includes(e.condition)),1050,false,true);
 ...
 for(const [name,center]of [['urban',[30.0466,-94.8852]],['rural',[30.115,-94.894]],['water',[30.019,-94.822]]]){
  ...
  if(name==='urban')await page.screenshot({path:out+'/complete-marker-map-density.png'});
