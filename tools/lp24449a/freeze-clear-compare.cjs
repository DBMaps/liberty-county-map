...
for(const baseline of [true,false]){
 const run=spawnSync(process.execPath,['tools/lp24449a/freeze-verify-browser.cjs'],{env:{...process.env,GRIDLY_49A_BASELINE:baseline?'1':'0',GRIDLY_49A_CLEAR_ONLY:'1',GRIDLY_49A_COMBINED_ONLY:'0'},encoding:'utf8',maxBuffer:10*1024*1024});
 fs.writeFileSync(`.artifacts/lp24449a/freeze-clear-${baseline?'baseline':'repaired'}.log`,run.stdout+'\n'+run.stderr);
 const evidence=JSON.parse(fs.readFileSync(`.artifacts/lp24449a/freeze-clear-${baseline?'baseline':'repaired'}/browser-390.json`));
...
fs.writeFileSync('reports/lp24449a/freeze-clear-comparison.json',JSON.stringify(result,null,2)+'\n');
