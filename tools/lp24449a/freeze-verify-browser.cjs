...
const output = path.join(root, process.env.GRIDLY_49A_CLEAR_ONLY === '1' ? (process.env.GRIDLY_49A_BASELINE === '1' ? '.artifacts/lp24449a/freeze-clear-baseline' : '.artifacts/lp24449a/freeze-clear-repaired') : (process.env.GRIDLY_49A_BASELINE === '1' ? '.artifacts/lp24449a/freeze-baseline' : '.artifacts/lp24449a/freeze'));
...
evidence.sourceHash = crypto.createHash('sha256').update(fs.readFileSync(process.env.GRIDLY_49A_BASELINE === '1' ? '.artifacts/lp24449a/baseline-app.js' : 'js/app.js')).digest('hex');
...
const server = http.createServer((req, res) => {
  ...
  const file = path.resolve(root, process.env.GRIDLY_49A_BASELINE === '1' && name === '/js/app.js' ? '.artifacts/lp24449a/baseline-app.js' : '.' + (name === '/' ? '/index.html' : name));
