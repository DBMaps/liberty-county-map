...
const required=name=>read(`reports/lp24449a/${name}.json`);
...
write(`${out}/cross-county-transitions.json`,ring);
...
const freeze=required('freeze-three-hazards'),settings=read('.artifacts/lp24449a/freeze/settings.json');
const geometry=read('.artifacts/lp24449a/freeze-geometry/browser.json');
...
const routes=['session.cjs','freeze-verify-browser.cjs','freeze-verify-settings.cjs','freeze-geometry.cjs'].map(name=>{const source=fs.readFileSync(`${root}/tools/lp24449a/${name}`,'utf8');return {file:`tools/lp24449a/${name}`,sha256:crypto.createHash('sha256').update(source).digest('hex'),methodAllowlist:/\['GET',\s*'HEAD',\s*'OPTIONS'\]\.includes/.test(source),serviceWorkersBlocked:/serviceWorkers:\s*['"]block['"]/.test(source),webSocketsClosed:source.includes('routeWebSocket')};});
...
write(`${out}/backend-safety.json`,backend);
...
write(`${out}/test-failure-details.json`,{repaired:repairedDetails,baseline:baselineDetails,changedReasons:changedTestFailureReasons});
...
write(`${out}/home-browser-results.json`,{completed:homes.length===cohort.total,sourceHash,count:homes.length,passed:homes.filter(r=>r.pass).length,records:homes});
for(const shard of [0,1]){
 ...
 write(`${out}/home-browser-results-${shard}.json`,{sourceHash,shard:`${shard}/2`,required,completed:records.length===required,count:records.length,passed:records.filter(row=>row.pass).length,records});
}
write(`${out}/replay-results.json`,{original:original.length,fixed:replay.filter(r=>r.status==='FIXED').length,remaining:replay.filter(r=>r.status!=='FIXED').length,records:replay});
...
write(`${out}/multicounty-recertification.json`,{communities:new Set(multi.map(r=>r.place_geoid)).size,memberships:multi.length,passed:multi.filter(r=>r.pass).length,records:multi});
...
write(`${out}/weather-locality-recertification.json`,weather);
...
write(`${out}/browser-summary.json`,browser);
...
gate('Observed Text Size limitation matches starting source',textSizeBaseline.sameControlMetrics&&textSizeBaseline.runs.find(run=>!run.baseline)?.sourceHash===sourceHash&&textSizeBaseline.runs.find(run=>run.baseline)?.sourceHash===required('starting-baseline').protectedFiles.find(file=>file.file==='js/app.js').sha256);
...
gate('Historical clear comparison uses exact starting source',clearComparison.runs.find(run=>run.baseline)?.sourceHash===required('starting-baseline').protectedFiles.find(file=>file.file==='js/app.js').sha256);
...
gate('Label oracle uses unchanged starting-source display formatter',labelBaseline.formatterUnchanged&&labelBaseline.sourceHash===sourceHash&&labelBaseline.baselineSourceHash===required('starting-baseline').protectedFiles.find(file=>file.file==='js/app.js').sha256);
...
write(`${out}/failure-ledger.json`,{decision,blockers,remainingOriginalFailures:replay.filter(r=>r.status!=='FIXED'),newHomeFailures:homes.filter(r=>!r.pass).map(r=>({county:r.county_id,place:r.place_geoid,checks:r.checks,error:r.error})),newSearchFailures:search.records.filter(r=>!r.pass),deferredLP24449B:deferred,liveLimitations:['No live external geocoding/provider acquisition certification.','Controlled NWS responses certify locality/request inputs, not live provider health.','Desktop Edge only; Android/iOS native WebView behavior requires device testing.']});
write(`${out}/decision.json`,{decision,sourceHash,blockers,originalFailures:369,fixed:replay.filter(r=>r.status==='FIXED').length,deferred,generatedAt:new Date().toISOString()});
