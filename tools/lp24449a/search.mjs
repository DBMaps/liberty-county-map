...
try {
 ...
 for(const row of inventory.rows.slice().sort((a,b)=>a.county_fips.localeCompare(b.county_fips)||a.place_geoid.localeCompare(b.place_geoid))){
  ...
  const started=Date.now();const entry=await page.evaluate(async row=>{
   ...
   if(result) {selectGridlySearchResult(result);await lp24449a.settle();}
   const context=lp24449a.identity(),weather=gridlyResolveGovernedWeatherPoint();
   const checks={bareDiscoverable:bare.some(r=>r.placeGeoid===row.place_geoid),qualifiedDiscoverable:!!result,
    ...
    homeBytesPreserved:JSON.stringify(bytes)===JSON.stringify(lp24449a.bytes())};
   return {...row,query,bare:bare.map(r=>({place:r.placeGeoid,county:r.requestedOperationalCountyId,title:r.title,provider:r.provider})),
  ...
  entry.runtimeMs=Date.now()-started;entry.completedAt=new Date().toISOString();records.push(entry);fs.appendFileSync(file,JSON.stringify(entry)+'\n');
