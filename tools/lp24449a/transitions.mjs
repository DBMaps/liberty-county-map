...
try {for(let i=0;i<representatives.length;i++){
 ...
 try{
  if(!ctx){ctx=await session.newPage();p=ctx.page;await p.addScriptTag({path:`${root}/tools/lp24449a/runtime-probe.js`});}
  entry.proof=await p.evaluate(async({home,target})=>{
   ...
   const saved=gridlySaveCanonicalMultiCountyPlaceHome(option.canonicalResolution,'lp24449a-ring',home.county_id);await lp24449a.settle();
   const before=lp24449a.bytes(),start=lp24449a.identity();
   ...
   if(!result)throw Error('Ring target missing');selectGridlySearchResult(result);await lp24449a.settle();
   const searched=lp24449a.identity();document.getElementById('gridlyTemporaryContextReturnHome').click();await lp24449a.settle();
   return {saved,start,searched,returned:lp24449a.identity(),weather:gridlyResolveGovernedWeatherPoint(),homeUnchanged:JSON.stringify(before)===JSON.stringify(lp24449a.bytes()),activeCounty:gridlyGetActiveCountyId(),crossingOwner:gridlyCrossingInventoryCountyId,driveOwner:window.gridlyLp028DriveTexasAreaLifecycleAudit?.().currentAwarenessViewCounty};
  },{home,target});
