...
const previous=process.argv.includes('--resume')&&fs.existsSync(`${out}/visible-search.json`)?read('reports/lp24449a/visible-search.json'):null;
const records=previous?.sourceHash===sourceHash?previous.records:[];let ctx,p;
const limit=Number(process.argv.find(a=>a.startsWith('--limit='))?.split('=')[1]||Infinity);
try {
 ctx=await session.newPage();p=ctx.page;await p.addScriptTag({path:`${root}/tools/lp24449a/runtime-probe.js`});await p.evaluate(()=>lp24449a.settle());
 ...
 for(const row of rows.filter(r=>!records.some(old=>old.place_geoid===r.place_geoid&&old.pass)).slice(0,limit)){const entry={...row};try{
  ...
  await p.evaluate(()=>lp24449a.settle());entry.context=await p.evaluate(()=>lp24449a.identity());
  await p.locator('#gridlySearchCloseBtn').evaluate(n=>n.click());
