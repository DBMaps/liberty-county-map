...
try{
 if(mode==='filter'){
  ...
  const search=await p.evaluate(async r=>{const results=await gridlySearchAddress(`${r.community_name}, ${r.county_name}, TX`),selected=results.find(x=>x.placeGeoid===r.place_geoid);selectGridlySearchResult(selected);await lp24449b.settle();return {selected,context:lp24449b.identity(),presentation:gridlyActiveGeographicPresentation};},row);
  const before=await raw(p);await p.locator('#gridlyPortraitV2 .gridly-v2-segments [data-geo-filter="county"]').first().click();await p.evaluate(()=>lp24449b.settle());
  const after=await raw(p),snapshot=await p.evaluate(()=>lp24449b.snapshot());
  ...
  for(const county of [...new Set(ledger.filter(r=>r.code==='PULSE_PUBLISHED_COUNT_STALE_AFTER_CLEAR').map(r=>r.county_id))]){
   ...
   try{
    await p.addScriptTag({path:`${root}/tools/lp24449b/runtime-probe.js`});await p.addScriptTag({path:`${root}/tools/lp24449b/original-sequence-probe.js`});
    await p.evaluate(r=>lp24449.home(r),row);
    await p.evaluate(async r=>{const q=await gridlySearchAddress(`${r.community_name}, ${r.county_name}, TX`);selectGridlySearchResult(q.find(x=>x.placeGeoid===r.place_geoid));await lp24449b.settle();},row);
    const filterObservations=[];
    for(const key of ['nearby','town','county','active-delays','all','county']){await p.locator(`#gridlyPortraitV2 .gridly-v2-segments [data-geo-filter="${key}"]`).first().click();await p.evaluate(()=>lp24449b.settle());filterObservations.push({key,originalSnapshot:await p.evaluate(()=>lp24449.snapshot())});}
    ...
    for(const kind of ['road','delay']){
     const fixture=await p.evaluate(async({kind,row,target})=>{const f=gridlyLocalTestReports.add(kind==='road'?'flooded-roadway':'reported-crossing-delay',kind==='road'?{lat:row.presentation_lat,lng:row.presentation_lng,ageMinutes:0}:{crossingId:target.id});await lp24449b.settle();return f;},{kind,row,target:original.render.target});
     const originalActiveObservation=await p.evaluate(()=>lp24449.snapshot());
     await p.locator('#gridlyAlertsDockButton').click();const originalAlertsObservation=await p.evaluate(()=>lp24449.snapshot()),active=await raw(p),activeModel=await p.evaluate(()=>lp24449b.snapshot());
     ...
     const beforeModel=await raw(p),model=await p.evaluate(()=>lp24449b.snapshot()),afterModel=await raw(p);
     await p.evaluate(()=>gridlyRefreshUnifiedAwarenessContext('lp24449b-normal-refresh'));await p.evaluate(()=>lp24449b.settle());const refreshed=await raw(p);
    ...
    await p.locator('#gridlyTemporaryContextReturnHome').evaluate(n=>n.click());await p.evaluate(()=>lp24449b.settle());const returned=await raw(p);
    ...
    if(originalTransition){afterCountyTransition=await p.evaluate(async query=>{const results=await gridlySearchAddress(query);if(!results[0])throw Error('Original county-transition query no longer resolves');selectGridlySearchResult(results[0]);await lp24449b.settle();const searched=lp24449b.snapshot();document.getElementById('gridlyTemporaryContextReturnHome').click();await lp24449b.settle();return {query,searched,returned:lp24449b.snapshot()};},originalTransition);}
...
if(prefix==='current'&&mode==='pulse'){
 ...
 const replay=spawnSync(process.execPath,['tools/lp24449b/baseline-replay.mjs','--pulse'],{stdio:'inherit'});
