...
const weatherReady=async()=>{if(awaitWeather)await p.waitForFunction(()=>{const r=gridlyWeatherConnectorRuntimeAudit();return r.pointsRequestAttempted===true&&r.requestAttempted===true&&r.requestInFlight===false;},null,{timeout:30000});};
const session=await require('./session.cjs')({fixtureWeather:true,fixtureReports:true});
let ctx,p,lastCounty;const networkRuns=[];
const hydrate=async()=>{await p.waitForFunction(()=>typeof gridlyGetCurrentAwarenessContext==='function'&&window.gridlyCanonicalCrossingRuntime?.state?.records);await p.addScriptTag({path:`${root}/tools/lp24449b/runtime-probe.js`});await p.evaluate(async()=>{await gridlyLoadStatewidePlacePresentation();await lp24449b.settle();window.lp24449b.homeSnapshot=()=>({activeCounty:gridlyGetActiveCountyId(),weatherPoint:gridlyResolveGovernedWeatherPoint(),weatherRuntime:window.gridlyWeatherConnectorRuntimeAudit?.(),driveArea:window.gridlyLp028DriveTexasAreaLifecycleAudit?.(),crossing:{owner:gridlyCrossingInventoryCountyId},location:document.getElementById('mobileAwarenessPanelKicker')?.textContent,kbyg:document.getElementById('gridlyBriefLocation')?.textContent,alertsAuthority:gridlyReadAlertsFamilyAuthority(),undefinedm:document.body.innerText.includes('undefinedm')});});};
try {for(const row of cohort.filter((r,index)=>index%shardCount===shardIndex&&(!only||only===`${r.county_id}|${r.place_geoid}`)&&(force||!done.has(`${r.county_id}|${r.place_geoid}`))).slice(0,limit)){
 ...
 try {
  ...
  entry.saved=await p.evaluate(()=>({identity:lp24449b.identity(),bytes:lp24449b.bytes()}));
  await p.locator('#gridlyPortraitV2SheetClose:visible, #closeSettingsModalBtn:visible').first().evaluate(n=>n.click());
  await p.reload({waitUntil:'domcontentloaded'});await hydrate();await weatherReady();
  entry.reloaded=await p.evaluate(()=>({identity:lp24449b.identity(),bytes:lp24449b.bytes(),snapshot:lp24449b.homeSnapshot()}));
  ...
  entry.transition=await p.evaluate(async target=>{const before=lp24449b.bytes();const results=await gridlySearchAddress(`${target.community_name}, ${target.county_name}, TX`);const result=results.find(r=>r.placeGeoid===target.place_geoid);if(!result)throw Error('Transition Search missing');selectGridlySearchResult(result);await lp24449b.settle();return {target,context:lp24449b.identity(),homeUnchanged:JSON.stringify(before)===JSON.stringify(lp24449b.bytes())};},target);
  await p.locator('#gridlyTemporaryContextReturnHome').evaluate(n=>n.click());await p.evaluate(()=>lp24449b.settle());
  await weatherReady();
  entry.returned=await p.evaluate(()=>({identity:lp24449b.identity(),bytes:lp24449b.bytes(),snapshot:lp24449b.homeSnapshot()}));
