...
const required=name=>read(`reports/${name==='original-in-scope-failures'?'lp24449a':'lp24449b'}/${name}.json`);
...
const freeze=required('freeze-three-hazards'),settings=read('.artifacts/lp24449b/freeze/settings.json');
const geometry=read('.artifacts/lp24449b/freeze-geometry/browser.json');
...
const routes=['session.cjs','freeze-verify-browser.cjs','freeze-verify-settings.cjs','freeze-geometry.cjs'].map(name=>{const source=fs.readFileSync(`${root}/tools/lp24449b/${name}`,'utf8');return {file:`tools/lp24449b/${name}`,sha256:crypto.createHash('sha256').update(source).digest('hex'),methodAllowlist:/\['GET',\s*'HEAD',\s*'OPTIONS'\]\.includes/.test(source),serviceWorkersBlocked:/serviceWorkers:\s*['"]block['"]/.test(source),webSocketsClosed:source.includes('routeWebSocket')};});
