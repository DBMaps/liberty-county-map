...
try {
 ...
 for(const failure of failures){const target=inv.rows.find(r=>r.place_geoid===failure.place_geoid&&r.county_id===failure.county_id);
  const proof=await page.evaluate(async target=>{const before=lp24449b.bytes(),baseline=lp24449b.identity();const results=await gridlySearchAddress(`${target.community_name}, ${target.county_name}, TX`);const result=results.find(r=>r.placeGeoid===target.place_geoid);if(!result)throw Error('Search missing');selectGridlySearchResult(result);await lp24449b.settle();const searched=lp24449b.identity();document.getElementById('gridlyTemporaryContextReturnHome').click();await lp24449b.settle();return {baseline,searched,returned:lp24449b.identity(),weather:gridlyResolveGovernedWeatherPoint(),storageUnchanged:JSON.stringify(before)===JSON.stringify(lp24449b.bytes())};},target);
