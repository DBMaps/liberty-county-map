...
try {
 ...
 await page.addScriptTag({path:`${root}/tools/lp24449b/runtime-probe.js`});
 await page.evaluate(()=>lp24449b.settle());
 await page.evaluate(async()=>{const group=getGridlyManualAwarenessAreaOptions().find(g=>g.countyId==='liberty-tx'),option=group.communities.find(o=>o.label==='Dayton');if(!gridlySaveCanonicalMultiCountyPlaceHome(option.canonicalResolution,'lp24449b-search-home-baseline',group.countyId))throw Error('Canonical Search Home baseline failed');await lp24449b.settle();});
 for(const row of inventory.rows.slice().sort((a,b)=>a.county_fips.localeCompare(b.county_fips)||a.place_geoid.localeCompare(b.place_geoid))){
  ...
  const started=Date.now();const entry=await page.evaluate(async row=>{
   ...
   if(result) {selectGridlySearchResult(result);await lp24449b.settle();}
   const filterBefore={context:lp24449b.identity(),activeCounty:gridlyGetActiveCountyId(),inventoryOwner:gridlyCrossingInventoryCountyId,presentation:gridlyActiveGeographicPresentation,center:map.getCenter(),anchor:getGridlyAwarenessAnchor()};
   if(result&&row.is_multi_county){document.querySelector('#gridlyPortraitV2 .gridly-v2-segments [data-geo-filter="county"]').click();await lp24449b.settle();}
   const filterAfter={context:lp24449b.identity(),activeCounty:gridlyGetActiveCountyId(),inventoryOwner:gridlyCrossingInventoryCountyId,resolution:gridlyCountyModeContextResolution,center:map.getCenter(),anchor:getGridlyAwarenessAnchor(),alertsOwner:Object.values(gridlyReadAlertsFamilyAuthority()).map(f=>f.owner),driveOwner:gridlyLp028DriveTexasAreaLifecycleAudit().currentAwarenessViewCounty};
   ...
   const context=lp24449b.identity(),weather=gridlyResolveGovernedWeatherPoint();
   const checks={bareDiscoverable:bare.some(r=>r.placeGeoid===row.place_geoid),qualifiedDiscoverable:!!result,
    ...
    homeBytesPreserved:JSON.stringify(bytes)===JSON.stringify(lp24449b.bytes())};
