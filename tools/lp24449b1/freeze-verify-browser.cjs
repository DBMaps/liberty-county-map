...
const output = path.join(root, process.env.GRIDLY_B1_OUTPUT || (process.env.GRIDLY_49A_CLEAR_ONLY === '1' ? (process.env.GRIDLY_49A_BASELINE === '1' ? '.artifacts/lp24449b1/freeze-clear-baseline' : '.artifacts/lp24449b1/freeze-clear-repaired') : (process.env.GRIDLY_49A_BASELINE === '1' ? '.artifacts/lp24449b1/freeze-baseline' : '.artifacts/lp24449b1/freeze')));
...
evidence.sourceHash = crypto.createHash('sha256').update(fs.readFileSync(process.env.GRIDLY_49A_BASELINE === '1' ? '.artifacts/lp24449b1/baseline-app.js' : 'js/app.js')).digest('hex');
evidence.interactionMethod = 'Rendered DOM controls invoke their production click handlers; post-click state and disclosure persistence are asserted. Pointer auto-wait timeouts are retained for both baseline and repaired runs.';
...
const server = http.createServer((req, res) => {
  ...
  const file = path.resolve(root, process.env.GRIDLY_49A_BASELINE === '1' && name === '/js/app.js' ? '.artifacts/lp24449b1/baseline-app.js' : process.env.GRIDLY_49A_BASELINE === '1' && name === '/js/gridlyWeatherLiveConnector.js' ? '.artifacts/lp24449b1/baseline-weather.js' : '.' + (name === '/' ? '/index.html' : name));
...
(async () => {
  ...
  const close=async()=>{if(await page.locator('#gridlyPortraitV2SheetClose').isVisible())await page.locator('#gridlyPortraitV2SheetClose').evaluate(n=>n.click());await page.locator('#gridlyPortraitV2Sheet').waitFor({state:'hidden'});};
  const open=async()=>{if(!await page.locator('#gridlyPortraitV2SheetClose').isVisible())await page.locator('#gridlyAlertsDockButton').evaluate(n=>n.click());await page.locator('#gridlyPortraitV2Sheet').waitFor({state:'visible'});};
  ...
  const clearRail=async(id='FRA-762785P')=>{await close();await page.evaluate(id=>{const c=crossings.find(c=>c.id===id);openCrossingPopupFromMarkerInteraction(crossingMarkers.get(id),c,'h1-ui-clear');},id);
    await page.waitForFunction(()=>!gridlyCrossingPopupOpeningAuditState.popupOpenPending&&!crossingRenderFrameHandle&&[...document.querySelectorAll('.leaflet-popup button')].some(n=>n.textContent.trim()==='Mark Cleared'&&n.isConnected&&!n.disabled));
    // A locator's resolved node can detach during a crossing render before
    // evaluate runs. Select and click the live DOM node in one browser task.
    const diagnostic=await page.evaluate(()=>{const n=[...document.querySelectorAll('.leaflet-popup button')].find(n=>n.textContent.trim()==='Mark Cleared'&&n.isConnected);if(!n||n.disabled)throw Error('Attached enabled Mark Cleared button missing');const events=[];const listener=e=>events.push({type:e.type,dataset:{...e.target.dataset},connected:e.target.isConnected});window.addEventListener('click',listener,true);const before={html:n.outerHTML,disabled:n.disabled,connected:n.isConnected,fixtures:gridlyLocalTestReports.list()};n.click();window.removeEventListener('click',listener,true);return {before,events,after:{html:n.outerHTML,fixtures:gridlyLocalTestReports.list()}};});
    evidence.clearActions??=[];evidence.clearActions.push(diagnostic);await page.evaluate(()=>map.closePopup());await page.waitForTimeout(500);};
  const matrix=async(label,expected,types={})=>{
    ...
    await page.locator('#gridlyBriefFoundationHandle').evaluate(n=>n.click());await page.waitForTimeout(400);s.kbyg=await page.locator('#gridlyBriefInteractionPanel').innerText();await shot(label+'-kbyg');await page.locator('#gridlyBriefFoundationHandle').evaluate(n=>n.click());
    ...
    for(const [id,type] of Object.entries(types)){
      ...
      s.popups.push({id,type,text});await shot(label+'-'+id+'-popup');await page.locator('.leaflet-popup-close-button').evaluate(n=>n.click());
    }
  };
  if (process.env.GRIDLY_49A_COMBINED_ONLY !== '1') {
  ...
  if (process.env.GRIDLY_49A_CLEAR_ONLY === '1') { evidence.passed = true; console.log('Historical crossing-clear diagnostic passed.'); return; }
  ...
  const community=page.locator('[data-gridly-disclosure-key="community_report"]');if(!await community.evaluate(n=>n.open))await community.locator(':scope > summary').evaluate(n=>n.click());
  ...
  await page.locator('#mobileDestinationCommandBtn').evaluate(n=>n.click());await page.locator('#gridlyAddressSearchInput').fill('Dayton');await page.locator('#gridlyRemoteSearchBtn').evaluate(n=>n.click());await page.locator('#gridlySearchResults button').filter({hasText:'Dayton'}).first().evaluate(n=>n.click());await page.locator('#gridlySearchCloseBtn').evaluate(n=>n.click());await record('combined-search',4);await page.locator('#gridlyTemporaryContextReturnHome').evaluate(n=>n.click());await record('combined-search-return',4);
  await page.evaluate(()=>{window.__h1ForegroundOriginal=requestGridlyForegroundPosition;requestGridlyForegroundPosition=(success,failure)=>{window.__h1Foreground={success,failure};return 'h1-local';};});await page.locator('#mobileDestinationCommandBtn').evaluate(n=>n.click());await page.locator('#gridlySearchAroundMeBtn').evaluate(n=>n.click());await page.evaluate(()=>window.__h1Foreground.success({timestamp:Date.now(),coords:{latitude:30.04725,longitude:-94.88737,accuracy:10}}));await record('combined-around',4);await page.locator('#gridlyTemporaryContextReturnHome').evaluate(n=>n.click());await page.evaluate(()=>requestGridlyForegroundPosition=window.__h1ForegroundOriginal);await record('combined-around-return',4);
  ...
  await community.locator(':scope > summary').evaluate(n=>n.click());await page.evaluate(()=>gridlyRunAlertsBackgroundRefreshAfterOpen('alerts_open_background_refresh'));await settled(4);assert.equal(await community.evaluate(n=>n.open),false);await close();await page.waitForTimeout(1200);assert.equal(await page.locator('#gridlyPortraitV2Sheet').isVisible(),false);await open();await settled(4);assert.equal(await community.evaluate(n=>n.open),false);await community.locator(':scope > summary').evaluate(n=>n.click());
  evidence.showMe=[];
  for(let i=0;i<5;i++){
    await page.locator('.gridly-lp236-show-me').first().evaluate(n=>n.click());
