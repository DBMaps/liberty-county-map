...
const pendingRows=cohort.filter((r,index)=>index%shardCount===shardIndex&&(!only||only===`${r.county_id}|${r.place_geoid}`)&&(force||!done.has(`${r.county_id}|${r.place_geoid}`))).slice(0,limit);
if(pendingRows.length===0){console.log('No remaining Home transactions in this shard; existing evidence preserved.');process.exit(0);}
...
try {for(const row of pendingRows){
