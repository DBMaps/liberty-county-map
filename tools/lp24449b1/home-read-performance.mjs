...
for(const baseline of [true,false]){
 ...
 try {
  ...
  const setup=await page.evaluate(async baseline=>{
   ...
   const saved=baseline?saveGridlyHomeTownPreference(option.value):gridlySaveCanonicalMultiCountyPlaceHome(option.canonicalResolution,'lp24449b1-home-read-performance',group.countyId);
   await lp24449b1.settle();
   return {saved,elapsedMs:performance.now()-start,context:lp24449b1.identity(),getterCalls:gridlySelectedAwarenessAreaResolutionCache.totalGetterCalls};
  },baseline);
  ...
  const sample=await page.evaluate(()=>{
   ...
   return {count,elapsedMs:performance.now()-start,area:{key:area?.key,county:area?.countyId,place:area?.placeGeoid},context:lp24449b1.identity()};
  });
  ...
  const run={baseline,sourceHash:crypto.createHash('sha256').update(fs.readFileSync(`${root}/${baseline?'.artifacts/lp24449b1/baseline-app.js':'js/app.js'}`)).digest('hex'),setup,sample,canonicalResolverSamples:profile.samples?.filter(id=>descendants.has(id)).length,totalSamples:profile.samples?.length,errors:evidence.errors};
