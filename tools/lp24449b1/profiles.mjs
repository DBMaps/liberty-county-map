...
try {for(const test of cases){
 ...
 const result=await page.evaluate(async()=>({identity:lp24449b1.identity(),weather:gridlyResolveGovernedWeatherPoint(),search:(await gridlySearchAddress('Frankston, Henderson County, TX')).map(r=>r.placeGeoid),bytes:lp24449b1.bytes(),chooser:!!document.querySelector('[data-v2-action="settings-change-home-area"]')}));
