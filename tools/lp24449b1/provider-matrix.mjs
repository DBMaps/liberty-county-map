...
try{
 ...
 const report=await p.evaluate(()=>gridlyLocalTestReports.add('flooded-roadway',{lat:gridlyGetCurrentAwarenessContext().lat,lng:gridlyGetCurrentAwarenessContext().lng,ageMinutes:0}));await p.evaluate(()=>lp24449b1.settle());await capture('community-active-unavailable');
 ...
 const officialRetention=await p.evaluate(async()=>{
  ...
  return {before,active,cleared,refreshed,fixture,pass:before.published===1&&active.published===2&&active.fresh===2&&[cleared,refreshed].every(r=>r.published===1&&r.fresh===1&&r.community.length===0&&r.official.length===1&&JSON.stringify(r.official)===JSON.stringify(before.official))};
 });
 write(`${out}/provider-health-matrix.json`,{states,officialRetention,network:s.evidence,passed:states.length===10&&states.every(s=>s.pass)&&officialRetention.pass&&s.evidence.errors.length===0});
