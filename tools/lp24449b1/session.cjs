...
module.exports=async function session(options={}){
  const server=http.createServer((req,res)=>{
    ...
    const file=path.resolve(root,options.baseline&&pathname==='/js/app.js'?'.artifacts/lp24449b1/baseline-app.js':options.baseline&&pathname==='/js/gridlyWeatherLiveConnector.js'?'.artifacts/lp24449b1/baseline-weather.js':'.'+(pathname==='/'?'/index.html':pathname));
  ...
  async function newPage(profile=null){
    ...
    await context.route('**/*',async route=>{
      ...
      if(options.providerFixture&&await options.providerFixture({route,url,evidence}))return;
