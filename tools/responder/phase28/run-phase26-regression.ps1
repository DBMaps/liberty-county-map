...
function Invoke-SqlFile([string]$file,[switch]$ExpectFailure){
  $text=[IO.File]::ReadAllText((Join-Path $historicalRoot $file))
  $text=$text.Replace('\ir seed-static-contract.sql',[IO.File]::ReadAllText((Join-Path $historicalRoot 'seed-static-contract.sql')))
  $text=$text.Replace('\ir compatibility.sql',[IO.File]::ReadAllText((Join-Path $historicalRoot 'compatibility.sql')))
...
try{
  ...
  try{
    ...
    & node --test (Join-Path $historicalRoot 'rehearsal-runtime.test.mjs')
    ...
    & node (Join-Path $historicalRoot 'catalog-snapshot.mjs') $raw (Join-Path $evidence 'post-catalog-certified.json')
