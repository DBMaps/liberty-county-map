...
export async function edgeHealth(env,fetchImpl=fetch) {
 const url=env.EDGE_URL;
 if(url!=='https://nhwhkbkludzkuyxmkkcj.supabase.co/functions/v1/gridly-cleanup-health') throw Error('monitor_configuration_missing');
 if(!/^[a-f0-9]{64}$/.test(env.GRIDLY_MONITOR_TOKEN||'')) throw Error('monitor_configuration_missing');
 const r=await fetchImpl(url,{method:'POST',redirect:'manual',signal:AbortSignal.timeout(10000),headers:{'X-Gridly-Monitor-Token':env.GRIDLY_MONITOR_TOKEN}});
...
export async function heartbeat(env,fetchImpl=fetch) {
 ...
 const r=await fetchImpl(env.DEADMAN_PING_URL,{method:'POST',redirect:'manual',signal:AbortSignal.timeout(8000),body:''});
 if(!r.ok) throw Error('deadman_failed');
}
export default {
 async fetch(){return new Response(null,{status:404});},
 async scheduled(_controller,env){
  try{await runScheduled({env,queryHealth:()=>edgeHealth(env)});if(env.DEADMAN_PING_URL!==undefined && env.DEADMAN_PING_URL!==null) await heartbeat(env);}
